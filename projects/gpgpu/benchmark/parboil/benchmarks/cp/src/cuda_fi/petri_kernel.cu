...
void fire_transition(char * g_places, int * conflict_array, int tr, int tc, int step, int N, int thd_thrd)
{
...
void compute_reward_stat(int * g_places, float * g_vars, int * g_maxs, int NSQUARE2)
{
    float sum = 0;
    int i;
    int max = 0;
...
__global__ void PetrinetKernel(int * g_s, float * g_v, int * g_m, int n, int s, int seed,  struct _gpufi_data_ *gpufi_dev)
{
     ...
     */
    int NSQUARE2 = ((n*n)*2);
    GPUFI_KERNEL_VARIABLE(gpufi_dev, 0, "NSQUARE2", (int*)&NSQUARE2);


    int * g_places = (g_s+(blockIdx.x*((NSQUARE2>>2)+NSQUARE2)));
    GPUFI_KERNEL_VARIABLE(gpufi_dev, 1, "g_places", (int*)&g_places);
