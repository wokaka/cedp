...
# 32
__loc_sc__(__text__,,) texture< float4, 1, cudaReadModeElementType>  texCoors; 
__loc_sc__(__text__,,) texture< float2, 1, cudaReadModeElementType>  texSprms; 
__loc_sc__(__text__,,) texture< float, 1, cudaReadModeElementType>  texWghts; 
...
auto int blid = (blockIdx.x); 
auto int thid = (threadIdx.x); 

auto int myWorkIndex = (((Offset + blid) << 6) + thid); 
if (!(thid)) { 
s_Work = d_Work[Offset + blid]; }  
__syncthreads(); 
auto uint4 myWork = s_Work; 

auto int localthid; auto int a1; auto int a2; auto int a3; auto int a4; 
GetBases(myWork.x, myWorkIndex, myWork.w, localthid, a1, a2, a3, a4); 


auto float Result = (0.0F); 
auto int Maxthid = __mul24(a1, __mul24(a2, __mul24(a3, a4))); 
if (localthid < Maxthid) 
{ 
auto int n1; auto int n2; auto int n3; auto int n4; 
auto int off1; auto int off2; auto int off3; auto int off4; 
auto int atom1; auto int atom2; auto int atom3; auto int atom4; 
...
auto float4 Atom1 = tex1D(texCoors, (float)atom1); 
auto float4 Atom2 = tex1D(texCoors, (float)atom2); 
auto float4 Atom3 = tex1D(texCoors, (float)atom3); 
auto float4 Atom4 = tex1D(texCoors, (float)atom4); 
auto float2 Param1 = tex1D(texSprms, (float)n1); 
auto float2 Param2 = tex1D(texSprms, (float)n2); 
auto float2 Param3 = tex1D(texSprms, (float)n3); 
auto float2 Param4 = tex1D(texSprms, (float)n4); 

auto float R12 = dist2(Atom1, Atom2); 
auto float R34 = dist2(Atom3, Atom4); 
auto float sum12 = (Param1.x + Param2.x); 
auto float sum34 = (Param3.x + Param4.x); 
auto float prod12 = (Param1.x * Param2.x); 
auto float prod34 = (Param3.x * Param4.x); 
auto float preexp = (__fdividef(prod12, sum12) * R12 + __fdividef(prod34, sum34) * R34); 

auto float preintegral = (__fdividef((((Param1.y * Param2.y) * Param3.y) * Param4.y) * __expf(-preexp), sum12 * sum34) * ((1) / sqrtf(sum12 + sum34))); 



if ((preintegral * preintegral) > (9.999999998e-24F)) 
{ 
auto float3 Atomp = ReducePair(Atom1, Atom2, Param1, Param2, sum12); 
auto float3 Atomq = ReducePair(Atom3, Atom4, Param3, Param4, sum34); 

auto float rpq2 = dist2(Atomp, Atomq); 
auto float rho = __fdividef(sum12 * sum34, sum12 + sum34); 
auto float weight = Root(rpq2 * rho); 
...
for (unsigned s = ((blockDim.x) >> 1); s > (0); s = s >> 1) 
{ 
if (thid < s) { 
...
auto uint2 myWork = (d_FinalReduce[blid + Offset]); 
firstElement = myWork.x; 
offset = myWork.y; 

if (thid < offset) { 
((Result)[thid]) = d_Output[firstElement + thid]; } else { 
