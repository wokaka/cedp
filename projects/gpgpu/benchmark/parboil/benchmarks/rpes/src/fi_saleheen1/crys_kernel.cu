...
__global__ void ComputeX(uint4*, float*, int, struct gpufi_fault *gpufi_fault, struct gpufi_profile *gpufi_profile, struct gpufi_current *gpufi_current);
...
__global__ 
void ComputeX(uint4* d_Work, float* d_Output, int Offset, struct gpufi_fault *gpufi_fault, struct gpufi_profile *gpufi_profile, struct gpufi_current *gpufi_current)
{
  ...
  int thid = threadIdx.x;
	...
	if(blid<1 && thid<1){
	GPUFI_KERNEL_KERNEL(0, gpufi_fault, gpufi_profile, gpufi_current);
	}
...
if(blid<1 && thid<1){
GPUFI_KERNEL_LOOP(gpufi_fault, gpufi_profile, gpufi_current);
}	
  for(unsigned int s = blockDim.x>>1; s > 0;  s = s>>1)
    {
	
	if(blid<1 && thid<1){
	GPUFI_KERNEL_ITERATION(gpufi_fault, gpufi_profile, gpufi_current);
	GPUFI_KERNEL_VARIABLE("blockDim.x", (int *)&blockDim.x, gpufi_fault, gpufi_profile, gpufi_current);
    GPUFI_KERNEL_VARIABLE("s", (int *)&s, gpufi_fault, gpufi_profile, gpufi_current);
    GPUFI_KERNEL_VARIABLE("thid", (int *)&thid, gpufi_fault, gpufi_profile, gpufi_current);
	}
...
{
    if((*gpufi_fault).mode == GPUFI_PROFILE){
        /* kernel */

        if(gpufi_kernel_bitmap[type] == 0){
            (*gpufi_profile).kernels++;
            gpufi_kernel_bitmap[type] = 1;
        }

        /* instance */
        (*gpufi_profile).instance[type]++;

        /* update current */
        (*gpufi_current).kernel = type;
        (*gpufi_current).instance = (*gpufi_profile).instance[type] - 1;
    }
    else{
        if(type == (*gpufi_fault).kernel_type){
            if((*gpufi_fault).kernel != type){
                (*gpufi_fault).kernel = (*gpufi_fault).kernel_count;
                GPUFI_KERNEL_SET(&(*gpufi_fault).instance, (*gpufi_fault).instance_count);
            }
            else{
                GPUFI_KERNEL_DEC(&(*gpufi_fault).instance);
            }
            GPUFI_KERNEL_SET(&(*gpufi_fault).loop, (*gpufi_fault).loop_count);
            GPUFI_KERNEL_SET(&(*gpufi_fault).iteration, (*gpufi_fault).iteration_count);
            
            (*gpufi_fault).disable = 0;
        }
        else{
            (*gpufi_fault).disable = 1;
        }
    }
}
...
{
    if((*gpufi_fault).mode == GPUFI_PROFILE){
        (*gpufi_profile).loop[(*gpufi_current).kernel][(*gpufi_current).instance]++;
        (*gpufi_current).loop = (*gpufi_profile).loop[(*gpufi_current).kernel][(*gpufi_current).instance] - 1;
    }
    else{
        if((*gpufi_fault).disable)
            return;
            
        GPUFI_KERNEL_DEC(&(*gpufi_fault).loop);
        GPUFI_KERNEL_SET(&(*gpufi_fault).iteration, (*gpufi_fault).iteration_count);
    }
}
...
{
    if((*gpufi_fault).mode == GPUFI_PROFILE){
        (*gpufi_profile).iteration[(*gpufi_current).kernel][(*gpufi_current).instance][(*gpufi_current).loop]++;
    }
    else{
        if((*gpufi_fault).disable)
            return;

        GPUFI_KERNEL_DEC(&(*gpufi_fault).iteration);
    }
}
...
void GPUFI_KERNEL_VARIABLE(char *name, int *variable, struct gpufi_fault *gpufi_fault, struct gpufi_profile *gpufi_profile, struct gpufi_current *gpufi_current)
{
    ...
    if(!(*gpufi_fault).injected){
        if((*gpufi_fault).instance == 0 && 
           (*gpufi_fault).loop == 0 &&
           (*gpufi_fault).iteration == 0 &&
	   !GPUFI_STRCMP((*gpufi_fault).variable_name, name)){
            (*gpufi_fault).injected = 1;
