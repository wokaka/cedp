...
__global__ void ComputeX(uint4*, float*, int, struct _gpufi_data_ *gpufi_dev);
...
__global__ 
void NumInteger(int* d_org, float *d_err, int count)
{
	int	i, j;
	int value, fault;

  RandomInit((uint32)d_org);
	for(i=0; i<count; i++){
	  value = BRandom();
	  RandomInit(value);
		for(j=0; j<32; j++){
			fault = value ^ (1 << j);
			d_org[i*32+j] = value;
			d_err[i*32+j] = fault - value;
		}
	}
}


__device__ char val[8];

__global__ 
void NumFloat_Gen(float* d_org, float *d_err, int count)
{
	int	i, j, ival;
	int rand;
	float value, fault, fval;
	float *fvalue;
...
__global__ 
void ComputeX(uint4* d_Work, float* d_Output, int Offset, struct _gpufi_data_ *gpufi_dev)
{
  ...
  GPUFI_KERNEL_KERNEL(gpufi_dev, 1);
	...
	GPUFI_KERNEL_VARIABLE(gpufi_dev, 1001, "", (int *)&d_Work);
//	GPUFI_KERNEL_VARIABLE_STREAM(gpufi_dev, 2001, "", (int *)d_Work, 3510150, 1);

	GPUFI_KERNEL_VARIABLE(gpufi_dev, 1002, "", (int *)&d_Output);
	GPUFI_KERNEL_VARIABLE_STREAM(gpufi_dev, 2002, "", (int *)d_Output, 3510150, 1);

	GPUFI_KERNEL_VARIABLE(gpufi_dev, 1003, "", (int *)&Offset);

	GPUFI_KERNEL_VARIABLE(gpufi_dev, 1004, "", (int *)&Data);
//	GPUFI_KERNEL_VARIABLE_STREAM(gpufi_dev, 2004, "", (int *)Data, BLOCK_SIZE, 1);


	GPUFI_KERNEL_VARIABLE(gpufi_dev, 1006, "", (int *)&blid);
	
	GPUFI_KERNEL_VARIABLE(gpufi_dev, 1007, "", (int *)&thid);
  
  int myWorkIndex = ((Offset + blid) << LOG_BLOCK_SIZE) + thid;

	GPUFI_KERNEL_VARIABLE(gpufi_dev, 1008, "", (int *)&myWorkIndex);

  if(!thid){
  	GPUFI_KERNEL_VARIABLE(gpufi_dev, 2001, "", (int *)&d_Work[Offset + blid]);
    s_Work = d_Work[Offset + blid];
  }

	GPUFI_KERNEL_VARIABLE(gpufi_dev, 1005, "", (int *)&s_Work);
  __syncthreads();
  uint4 myWork = s_Work;
  ...
  GetBases(myWork.x, myWorkIndex, myWork.w,
	...
	GPUFI_KERNEL_VARIABLE(gpufi_dev, 1009, "", (int *)&localthid);

	GPUFI_KERNEL_VARIABLE(gpufi_dev, 1010, "", (int *)&a1);

  float Result = 0.0f;
  int Maxthid = __mul24(a1, __mul24(a2, __mul24(a3, a4)));

	GPUFI_KERNEL_VARIABLE(gpufi_dev, 1011, "", (int *)&Result);
  ...
  if(localthid < Maxthid)
    {
      ...
      GetOffsets(myWork.z, off1, off2, off3, off4);
			...
			GPUFI_KERNEL_VARIABLE(gpufi_dev, 1012, "", (int *)&n1);

			GPUFI_KERNEL_VARIABLE(gpufi_dev, 1013, "", (int *)&off2);
      ...
      n4 += off4;
			...
			GPUFI_KERNEL_VARIABLE(gpufi_dev, 1014, "", (int *)&atom4);
      ...
      float2 Param4 = tex1D(texSprms, (float)n4);
			...
			GPUFI_KERNEL_VARIABLE(gpufi_dev, 1015, "", (int *)&Atom3);

			GPUFI_KERNEL_VARIABLE(gpufi_dev, 1016, "", (int *)&Param2);

      float R12 = dist2(Atom1, Atom2);
			GPUFI_KERNEL_VARIABLE(gpufi_dev, 1017, "", (int *)&R12);
			
      float R34 = dist2(Atom3, Atom4);
      float sum12  = Param1.x + Param2.x;
      float sum34  = Param3.x + Param4.x;
			GPUFI_KERNEL_VARIABLE(gpufi_dev, 1018, "", (int *)&sum34);
      float prod12 = Param1.x * Param2.x;
      float prod34 = Param3.x * Param4.x;
			GPUFI_KERNEL_VARIABLE(gpufi_dev, 1019, "", (int *)&prod34);
      float preexp = __fdividef(prod12, sum12) * R12 + __fdividef(prod34, sum34) * R34;
			GPUFI_KERNEL_VARIABLE(gpufi_dev, 1020, "", (int *)&preexp);
      ...
      if(preintegral * preintegral > 1.0e-23f)
			{
		  	...
		  	GPUFI_KERNEL_VARIABLE(gpufi_dev, 1021, "", (int *)&Atomp);
			  
			  float rpq2 = dist2(Atomp, Atomq);
		
		  	GPUFI_KERNEL_VARIABLE(gpufi_dev, 1022, "", (int *)&rpq2);
		
			  float rho = __fdividef(sum12 * sum34, sum12 + sum34);
			  float weight = Root(rpq2 * rho);
		
		  	GPUFI_KERNEL_VARIABLE(gpufi_dev, 1023, "", (int *)&weight);
...
//  unsigned int blockDim_x_temp = blockDim.x;

  for(unsigned int s = blockDim.x>>1; s > 0;  s = s>>1)
//  for(unsigned int s = blockDim_x_temp>>1; s > 0;  s = s>>1)
    {
      GPUFI_KERNEL_ITERATION(gpufi_dev);
	  	GPUFI_DEBUG_LOC(gpufi_dev);

     
      if (thid < s){
//  			GPUFI_KERNEL_VARIABLE(gpufi_dev, 2004, "", (int *)&Data[thid+s]);
        Data[thid] += Data[thid+s];
  			GPUFI_KERNEL_VARIABLE(gpufi_dev, 2004, "", (int *)&Data[thid]);
      }
...
//			GPUFI_KERNEL_VARIABLE_STREAM(gpufi_dev, 2004, "", (int *)Data, BLOCK_SIZE, 1);
  ...
  if(!thid)
    d_Output[Offset + blid] = Data[0];
 	 ...
 	 GPUFI_KERNEL_VARIABLE(gpufi_dev, 2002, "", (int *)&d_Output[Offset + blid]);
//  GPUFI_KERNEL_OUTPUT(gpufi_dev, d_Output, );
}

__global__ 
void DoReduction(float* d_ReductionSum, float* d_Output, 
			    uint2* d_FinalReduce, int block_size, int Offset, struct _gpufi_data_ *gpufi_dev)
{
  int firstElement;
  int offset;
  __shared__ float Result[BLOCK_SIZE];
  __shared__ float Result_dup[BLOCK_SIZE];
  ...
  if(!thid){
//    GPUFI_KERNEL_VARIABLE(gpufi_dev, 31, "d_ReductionSum[...]", (int *)&d_ReductionSum[blid + Offset]);
  }
  ...
  for(unsigned int s = blockDim.x>>1; s > 0;  s = s>>1)
    {
      ...
      if (thid < s){
//        GPUFI_KERNEL_VARIABLE(gpufi_dev, 30, "Result[thid]", (int *)&Result[thid]);
...
//    GPUFI_KERNEL_VARIABLE_FORCE(gpufi_dev, 31, "d_ReductionSum[...]", (int *)&d_ReductionSum[blid + Offset]);
  }
}
