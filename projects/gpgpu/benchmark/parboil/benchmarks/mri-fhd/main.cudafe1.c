...
# 36 "src/cuda_fi/computeFH.cu"
extern void _Z17createDataStructsiiPPfS0_S0_S0_(int, int, float **, float **, float **, float **);
# 412 "src/cuda_fi/computeFH.cu"
extern void _Z17computeRhoPhi_GPUiPfS_S_S_S_S_(int, float *, float *, float *, float *, float *, float *);
# 448 "src/cuda_fi/computeFH.cu"
...
static void _Z14setupMemoryGPUiiPPfS0_(int, int, float **, float **);
# 72 "src/cuda_fi/main.cu"
static void _Z16cleanupMemoryGPUiiPPfS0_(int, int, float **, float **);
# 96 "src/cuda_fi/main.cu"
extern int main(int, char **);
static void __sti___12_main_cpp1_ii_46b07679(void) __attribute__((__constructor__));
# 73 "src/cuda_fi/gpufi.h"
...
memset(((void *)(&gpufi_host)), 0, 20192U);
...
printf("kernel\t%d\tinstance\t%d\tcall\t%d\n", (((((struct _gpufi_profile_kernel_ *)(&(gpufi_host.profile).kernel))[i]).id)), (((((struct _gpufi_profile_kernel_ *)(&(gpufi_host.profile).kernel))[i]).instance)), (((((struct _gpufi_profile_variable_ *)(((struct _gpufi_profile_variable_ (*)[128])(&(gpufi_host.profile).variable)) + i))[((gpufi_host.fault).varid)]).call_count))); } else  {
...
# 36 "src/cuda_fi/computeFH.cu"
void _Z17createDataStructsiiPPfS0_S0_S0_( int numK,  int numX,  float **realRhoPhi,  float **imagRhoPhi,  float **outR,  float **outI)
{
...
# 412 "src/cuda_fi/computeFH.cu"
void _Z17computeRhoPhi_GPUiPfS_S_S_S_S_( int numK,  float *phiR_d,  float *phiI_d,  float *dR_d,  float *dI_d,  float *realRhoPhi_d,  float *imagRhoPhi_d)
{
auto int rhoPhiBlocks;
auto dim3 DimRhoPhiBlock;
auto dim3 DimRhoPhiGrid;
# 414 "src/cuda_fi/computeFH.cu"
rhoPhiBlocks = (numK / 512);
{ (DimRhoPhiBlock.x) = 1U; (DimRhoPhiBlock.y) = 1U; (DimRhoPhiBlock.z) = 1U; }
{ (DimRhoPhiGrid.x) = 1U; (DimRhoPhiGrid.y) = 1U; (DimRhoPhiGrid.z) = 1U; }
if (numK % 512)
{
rhoPhiBlocks++;
}
(DimRhoPhiBlock.x) = 512U;
(DimRhoPhiBlock.y) = 1U;
(DimRhoPhiBlock.z) = 1U;
(DimRhoPhiGrid.x) = ((unsigned)rhoPhiBlocks);
(DimRhoPhiGrid.y) = 1U;
(DimRhoPhiGrid.z) = 1U;
# 432 "src/cuda_fi/computeFH.cu"
cudaMalloc(((void **)(&__shadow_var(gpufi_dev,gpufi_dev))), 20192U);
{ auto cudaError_t err;
# 433 "src/cuda_fi/computeFH.cu"
if ((int)(err = (cudaGetLastError()))) { fprintf(stderr, "CUDA error on line %d: %s\n", 433, (cudaGetErrorString(err))); exit((-1)); } }
cudaMemcpy(((void *)__shadow_var(gpufi_dev,gpufi_dev)), ((const void *)(&gpufi_host)), 20192U, cudaMemcpyHostToDevice);
{ auto cudaError_t err;
# 435 "src/cuda_fi/computeFH.cu"
if ((int)(err = (cudaGetLastError()))) { fprintf(stderr, "CUDA error on line %d: %s\n", 435, (cudaGetErrorString(err))); exit((-1)); } }


((int)(cudaConfigureCall(DimRhoPhiGrid, DimRhoPhiBlock, 0U, 0))) ? ((void)0) : (__device_stub__Z16ComputeRhoPhiGPUiPfS_S_S_S_S_P12_gpufi_data_(numK, phiR_d, phiI_d, dR_d, dI_d, realRhoPhi_d, imagRhoPhi_d, __shadow_var(gpufi_dev,gpufi_dev)));

cudaMemcpy(((void *)(&gpufi_host)), ((const void *)__shadow_var(gpufi_dev,gpufi_dev)), 20192U, cudaMemcpyDeviceToHost);
{ auto cudaError_t err;
# 441 "src/cuda_fi/computeFH.cu"
if ((int)(err = (cudaGetLastError()))) { fprintf(stderr, "CUDA error on line %d: %s\n", 441, (cudaGetErrorString(err))); exit((-1)); } }
cudaFree(((void *)__shadow_var(gpufi_dev,gpufi_dev)));
{ auto cudaError_t err;
# 443 "src/cuda_fi/computeFH.cu"
if ((int)(err = (cudaGetLastError()))) { fprintf(stderr, "CUDA error on line %d: %s\n", 443, (cudaGetErrorString(err))); exit((-1)); } } 


}

void _Z13computeFH_GPUiiPfS_S_P7kValuesS_S_( int numK,  int numX,  float *x_d,  float *y_d,  float *z_d,  struct kValues *kVals,  float *outR_d,  float *outI_d)
{
auto int FHGrids;
auto int FHBlocks;
...
auto dim3 DimFHGrid;
auto int FHGrid;
# 450 "src/cuda_fi/computeFH.cu"
FHGrids = (numK / 512);

{ (DimFHBlock.x) = 1U; (DimFHBlock.y) = 1U; (DimFHBlock.z) = 1U; }
{ (DimFHGrid.x) = 1U; (DimFHGrid.y) = 1U; (DimFHGrid.z) = 1U; }
...
cudaMalloc(((void **)(&__shadow_var(gpufi_dev,gpufi_dev))), 20192U);
{ auto cudaError_t err;
# 493 "src/cuda_fi/computeFH.cu"
if ((int)(err = (cudaGetLastError()))) { fprintf(stderr, "CUDA error on line %d: %s\n", 493, (cudaGetErrorString(err))); exit((-1)); } }
cudaMemcpy(((void *)__shadow_var(gpufi_dev,gpufi_dev)), ((const void *)(&gpufi_host)), 20192U, cudaMemcpyHostToDevice);
{ auto cudaError_t err;
# 495 "src/cuda_fi/computeFH.cu"
if ((int)(err = (cudaGetLastError()))) { fprintf(stderr, "CUDA error on line %d: %s\n", 495, (cudaGetErrorString(err))); exit((-1)); } }


((int)(cudaConfigureCall(DimFHGrid, DimFHBlock, 0U, 0))) ? ((void)0) : (__device_stub__Z13ComputeFH_GPUiiPfS_S_S_S_P12_gpufi_data_(numK, FHGridBase, x_d, y_d, z_d, outR_d, outI_d, __shadow_var(gpufi_dev,gpufi_dev)));

cudaMemcpy(((void *)(&gpufi_host)), ((const void *)__shadow_var(gpufi_dev,gpufi_dev)), 20192U, cudaMemcpyDeviceToHost);
{ auto cudaError_t err;
# 501 "src/cuda_fi/computeFH.cu"
if ((int)(err = (cudaGetLastError()))) { fprintf(stderr, "CUDA error on line %d: %s\n", 501, (cudaGetErrorString(err))); exit((-1)); } }
cudaFree(((void *)__shadow_var(gpufi_dev,gpufi_dev)));
{ auto cudaError_t err;
# 503 "src/cuda_fi/computeFH.cu"
...
# 48 "src/cuda_fi/main.cu"
static void _Z14setupMemoryGPUiiPPfS0_( int num,  int size,  float **dev_ptr,  float **host_ptr)
{
...
static void _Z16cleanupMemoryGPUiiPPfS0_( int num,  int size,  float **dev_ptr,  float **host_ptr)
{
...
auto float *x;
# 108 "src/cuda_fi/main.cu"
auto float *y;
# 108 "src/cuda_fi/main.cu"
auto float *z;

auto float *phiR;
# 110 "src/cuda_fi/main.cu"
auto float *phiI;

auto float *dR;
# 112 "src/cuda_fi/main.cu"
auto float *dI;

auto float *realRhoPhi;
# 114 "src/cuda_fi/main.cu"
auto float *imagRhoPhi;

auto float *outI;
# 116 "src/cuda_fi/main.cu"
auto float *outR;
...
# 101 "src/cuda_fi/main.cu"
_Z10GPUFI_INITii(2, 84);
# 127 "src/cuda_fi/main.cu"
...
_Z17createDataStructsiiPPfS0_S0_S0_(numK, numX, (&realRhoPhi), (&imagRhoPhi), (&outR), (&outI));
kVals = ((struct kValues *)(calloc(((size_t)numK), 20U)));

{

auto float *phiR_d;
# 166 "src/cuda_fi/main.cu"
auto float *phiI_d;
auto float *dR_d;
# 167 "src/cuda_fi/main.cu"
auto float *dI_d;
auto float *realRhoPhi_d;
# 168 "src/cuda_fi/main.cu"
auto float *imagRhoPhi_d;
pb_SwitchToTimer((&timers), pb_TimerID_COPY);
_Z14setupMemoryGPUiiPPfS0_(numK, 4, (&phiR_d), (&phiR));
_Z14setupMemoryGPUiiPPfS0_(numK, 4, (&phiI_d), (&phiI));
_Z14setupMemoryGPUiiPPfS0_(numK, 4, (&dR_d), (&dR));
_Z14setupMemoryGPUiiPPfS0_(numK, 4, (&dI_d), (&dI));
...
pb_SwitchToTimer((&timers), pb_TimerID_COPY);
_Z16cleanupMemoryGPUiiPPfS0_(numK, 4, (&realRhoPhi_d), (&realRhoPhi));
_Z16cleanupMemoryGPUiiPPfS0_(numK, 4, (&imagRhoPhi_d), (&imagRhoPhi));
...
_Z14setupMemoryGPUiiPPfS0_(numX, 4, (&x_d), (&x));
_Z14setupMemoryGPUiiPPfS0_(numX, 4, (&y_d), (&y));
_Z14setupMemoryGPUiiPPfS0_(numX, 4, (&z_d), (&z));
...
_Z16cleanupMemoryGPUiiPPfS0_(numX, 4, (&outR_d), (&outR));
_Z16cleanupMemoryGPUiiPPfS0_(numX, 4, (&outI_d), (&outI));
cudaFree(((void *)x_d));
cudaFree(((void *)y_d));
cudaFree(((void *)z_d));
}
