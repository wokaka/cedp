...
public class IVSubstitution extends TransformPass {
     ...
     */
    private Map<Loop, List<Expression>> loop_to_bounds = new LinkedHashMap<Loop, List<Expression>>();
    /**
     ...
     * expression.
     */
    private Map<Loop, Map<Statement, Expression>> loop_to_stmts = new LinkedHashMap<Loop, Map<Statement, Expression>>();
    ...
    private Map<Statement, Expression> ivstmt_to_inc = new LinkedHashMap<Statement, Expression>();
    /**
    ...
    private Map<Loop, Set<Statement>> loop_to_wrap_stmts = new LinkedHashMap<Loop, Set<Statement>>();
    /**
     ...
     */
    private Map<Symbol, Boolean> iv_to_divisible = new LinkedHashMap<Symbol, Boolean>();
    /**
     * Map form induction variables to the set of loops that enclose the
     * variables' induction statements.
     */
    private Map<Symbol, Set<Loop>> iv_to_loops = new LinkedHashMap<Symbol, Set<Loop>>();
    /**
     ...
     */
    private Map<Statement, Expression> inc_after_stmt = new LinkedHashMap<Statement, Expression>();
    /**
     ...
     */
    private Map<Loop, Expression> inc_into_loop = new LinkedHashMap<Loop, Expression>();
    /**
    ...
    public String getPassName() {
        return "[Induction Variable Substitution]";
    }
    ...
    public void start() {
        ...
        while (iter.hasNext()) {
            ...
            if (o instanceof TranslationUnit) {
                Tools.printlnStatus(tag + "Entering translation unit \""
                ...
                Tools.printlnStatus(tag + "Entering procedure \"" + proc.getName()
                        + "\"...", 1);
                ...
                if (result == WRAP) // Wrap-around variable has been detected.
                {
                    ...
                    range_map = RangeAnalysis.getRanges(((Statement) loop).getProcedure());
                    result = analyze(loop);
                }
    ...
    private int analyze(Loop loop) {
        if (Tools.containsClasses(loop, fail_fast_class)) {
            Tools.printStatus(tag + "Analysis stops due to an intractable loop: ", 1);
            Tools.printlnStatus(getReport(loop), 1);
            return FAIL;
        }
        ...
        for (Loop curr : eligible_loops) {
            ...
            RangeDomain rd = range_map.get(curr);
        ...
        for (int i = 0; i < eligible_loops.size(); i++) {
            ...
            if (i > 0 && !Tools.isAncestorOf(
                    curr, eligible_loops.get(i - 1))) {
                iv_placed.clear();
            }
            ...
            while (variants.size() != variants_size) {
                ...
                for (Statement stmt : new LinkedHashSet<Statement>(iv_stmts.keySet())) {
                    ...
                    if (iv_placed.get(iv) != null
                            && Tools.isAncestorOf(curr, iv_placed.get(iv))
                            || variants.contains(iv)
                            || Tools.containsSymbols(ivstmt_to_inc.get(stmt), variants)) {
            ...
            for (Statement stmt : iv_stmts.keySet()) {
                ...
                vars_with_ivs.addAll(Tools.getAccessedSymbols(iv_stmts.get(stmt)));
                ...
                while (true) {
                    if (t instanceof Loop && eligible_loops.contains(t)) {
                        ...
                        if (Tools.containsSymbols(loop_variants, vars_with_ivs)) {
                            break;
                        }
                        ...
                        vars_with_ivs.addAll(Tools.getAccessedSymbols(loop_bounds.get(1)));
                        vars_with_ivs.addAll(Tools.getAccessedSymbols(loop_bounds.get(2)));
                        vars_with_ivs.addAll(Tools.getAccessedSymbols(loop_bounds.get(3)));
                ...
                if (is_divisible == null || is_divisible == true) {
                    if (Tools.containsBinary(stmt, BinaryOperator.DIVIDE)
                            || Tools.containsBinary(stmt, BinaryOperator.MODULUS)) {
        ...
        if (debug > 0) {
            Tools.printlnStatus(tag + "Analysis result ...", 1);
            reportAnalysis(loop, "");
        }
    ...
    private Expression getNZTCondition(Loop loop) {
        Expression ret = null;
        RangeDomain rd = range_map.get(loop);
    ...
    private Expression getNZTConditions(Loop loop) {
        ...
        for (int i = 0; i <= eligible_loops.indexOf(loop); i++) {
            ...
            if (Tools.isAncestorOf(loop, curr_loop)
    ...
    private void reportAnalysis(Loop loop, String indent) {
        ...
        Tools.printlnStatus(tag + indent + getReport(loop), 1);
        indent += "  ";
        ...
        while (iter.hasNext()) {
            ...
            if (o instanceof Loop) {
                ...
                Tools.printlnStatus(tag + indent + o + " [inc="
        ...
        if (loop == eligible_loops.getLast()) {
            for (Symbol iv : iv_to_loops.keySet()) {
                Tools.printlnStatus(tag + "ivs=" + iv.getSymbolName() + ":", 1);
                Set<Loop> iv_loops = iv_to_loops.get(iv);
                for (Loop iv_loop : iv_loops) {
                    Tools.printlnStatus(tag + "    " + getReport(iv_loop), 1);
                }
            }
        }
    }
    ...
    private int matchInduction(Loop loop) {
        ...
        if (!LoopTools.isCanonical(loop)) {
            Tools.printStatus(tag + "Analysis stops due to a non-canonical loop: ", 1);
            Tools.printlnStatus(getReport(loop), 1);
            return FAIL;
        }
        ...
        while (iter.hasNext()) {
            ...
            if (o instanceof Loop) {
                ...
                {                           // part of the inner loops.
                    ...
                    variants.addAll(Tools.getDefSymbol(forloop.getInitialStatement()));
                    variants.addAll(Tools.getDefSymbol(forloop.getCondition()));
                    variants.addAll(Tools.getDefSymbol(forloop.getStep()));
                }
            ...
            {    // which includes any kinds of conditional branches.
                variants.addAll(Tools.getDefSymbol((Traversable) o));
                unsafe_stmts.add((Statement) o);
            }
        }

        // Add locally (loop) defined symbols in the set of variants.
        variants.addAll(Tools.getSymbols((SymbolTable) loop.getBody()));
    ...
    private void matchInduction(Loop loop, ExpressionStatement stmt) {
        ...
        if (e instanceof AssignmentExpression) {
            ...
            loop_to_variants.get(loop).addAll(Tools.getDefSymbol(e));
        }
    }
    ...
    private void matchAssignment(Loop loop, AssignmentExpression e) {
        ...
        Symbol var = Tools.getSymbolOf(lhs);

        if (lhs instanceof Identifier && Tools.isInteger(var)) {
            Expression inc = null, zero = new IntegerLiteral(0);
            if (op.equals("=")
                    && !Tools.containsSymbol(inc = Symbolic.subtract(rhs, lhs), var)
                    || op.equals("+=")
                    && !Tools.containsSymbol(inc = Symbolic.simplify(rhs), var)
                    || op.equals("-=")
                    && !Tools.containsSymbol(inc = Symbolic.subtract(zero, rhs), var)) {
                ivstmt_to_inc.put(stmt, inc);
                Set<Symbol> vars_in_inc = Tools.getAccessedSymbols(inc);
                inc = range_map.get(stmt).substituteForward(inc, vars_in_inc);
                loop_to_stmts.get(loop).put(stmt, inc);
                Tools.printlnStatus(tag + "Found " + stmt + " in " + getReport(loop), 1);
            } else {
                loop_to_variants.get(loop).addAll(Tools.getDefSymbol(e));
            }
        } else {
    ...
    private void matchUnary(Loop loop, UnaryExpression e) {
        ...
        String op = e.getOperator().toString();
        Symbol var = Tools.getSymbolOf(ue);

        if (ue instanceof Identifier && Tools.isInteger(var)
                && (op.equals("++") || op.equals("--"))) {
    ...
    private Symbol getIV(Statement stmt) {
        ...
        if (e instanceof AssignmentExpression) {
            return Tools.getSymbolOf(((AssignmentExpression) e).getLHS());
        } else if (e instanceof UnaryExpression) {
            return Tools.getSymbolOf(((UnaryExpression) e).getExpression());
        } else {
    ...
    private List<Symbol> getSubstOrder(Loop loop) {
        ...
        for (Symbol iv : ivs) {
            ...
            for (Loop iv_loop : iv_loops) {
                ...
                for (Statement stmt : iv_stmts.keySet()) {
                    ...
                    Set<Symbol> accessed = Tools.getAccessedSymbols(inc);
        ...
        for (DFANode root : iv_graph.getEntryNodes()) {
            for (Object component : iv_graph.getSCC(root)) {
                if (component instanceof List && ((List<DFANode>) component).size() == 1) {
                    DFANode iv_node = ((List<DFANode>) component).get(0);
                    Symbol iv_symbol = (Symbol) iv_node.getData("iv");
                    if (!ret.contains(iv_symbol)) {
                        ret.add(iv_symbol);
                    }
                } else {
                    Tools.printlnStatus(tag + "Found cyclic induction variables", 1);
                    ret.clear();
                    break;
                }
            }
    ...
    private int transform(Loop loop) {
        Tools.printlnStatus(tag + "Transformation starts...", 1);
        ...
        if (nzt_condition != null) {
            orig = (Statement) ((Statement) loop).clone();
        }
        ...
        for (int i = 0; i < eligible_loops.size(); ++i) {
            ...
            for (Symbol iv : ivs) {
                ...
                if (replace(curr_loop, iv, init) == PASS) {
                    Tools.printlnStatus(tag + "Successfully replaced " + iv.getSymbolName()
            ...
            for (int j = 0; j <= i; ++j) {
                Set<Statement> iv_stmts = new LinkedHashSet<Statement>(loop_to_stmts.get(eligible_loops.get(j)).keySet());
        ...
        if (nzt_condition != null) {
            Statement transformed = (Statement) ((Statement) loop).clone();
    ...
    private Expression findIncrement(Loop loop, Symbol iv) {
        ...
        while (iter.hasNext()) {
            ...
            if (o instanceof Loop) {
                ...
                if (eligible_loops.contains(inner)) {
                    ...
                    Tools.printlnStatus(tag + "inc_after[" + getReport(inner) + "] = " + inc, 1);
                ...
                Tools.printlnStatus(tag + "inc_after[" + o + "] = " + inc, 1);
        ...
        Tools.printlnStatus(tag + "inc_into[" + getReport(loop) + "] = " + sum, 1);
     ...
     */
    private static boolean containsAncestor(Set<? extends Traversable> ancs, Traversable t) {
        for (Traversable anc : ancs) {
            if (Tools.isAncestorOf(anc, t)) {
                return true;
            }
        }
        return false;
    ...
    private static boolean containsDescendant(Set<? extends Traversable> dess, Traversable t) {
        for (Traversable des : dess) {
            if (Tools.isAncestorOf(t, des)) {
                return true;
            }
        }
        return false;
        ...
        while (iter.hasNext()) {
            ...
            if (o instanceof Loop) {
                ...
                if (loop_to_stmts.get(inner) == null) {
                    Tools.replaceSymbolIn(inner, iv, val);
                    replaced.add(inner);
                ...
                {
                    Tools.replaceSymbolIn((Traversable) o, iv, val);
                    replaced.add((Traversable) o);
                }
                // Update the increment cache as well.
                loop_to_stmts.get(loop).put(iv_stmt, Tools.replaceSymbol(inc, iv, val));
    ...
    private Expression getInitialValue(Loop loop, Symbol iv) {
        ...
        for (int i = siblings.indexOf(loop) - 1; i >= 0; i--) {
            ...
            if (Tools.containsClass(elder, FunctionCall.class)) {
                return null;
            }
            if (Tools.getDefSymbol(elder).contains(iv)) {
    ...
    private boolean isWrapAroundVariable(Loop loop, Symbol var) {
        ...
        RangeDomain before_rd = range_map.get(loop);
        ...
        if (before_range == null
                ...
                || Tools.containsSymbols(before_range, variants)) {
            return false;
        }
        ...
        for (int i = stmts_in_loop.size() - 1; i >= 0; i--) {
            ...
            if (t instanceof ExpressionStatement
                    && ((ExpressionStatement) t).getExpression() instanceof AssignmentExpression) {
                AssignmentExpression assign =
                        (AssignmentExpression) ((ExpressionStatement) t).getExpression();
                if (Tools.getSymbolOf(assign.getLHS()) == var
                        && !Tools.containsSymbols(assign.getRHS(), variants)
                    ...
                    Tools.printlnStatus(tag + "Found wrap-around " + last_value_stmt
                            + " in " + getReport(loop), 1);
                    break;
                }
            } else {
    ...
    private void removeWrapAroundVariables() {
        ...
        for (Loop loop : eligible_loops) {
            ...
            parent.addStatementBefore(for_loop, (Statement) init_stmt.clone());
            ...
            while (iter.hasNext()) {
                ...
                if (!wrap_stmts.contains(child_stmt)) {
                    parent.addStatementBefore(for_loop, (Statement) child_stmt.clone());
                }
            }
            ...
            for (Statement wrap_stmt : wrap_stmts) {
                ...
                Statement moved_stmt = (Statement) wrap_stmt.clone();
                Tools.replaceSymbolIn(
            ...
            Expression ub = loop_to_bounds.get(loop).get(2);
            for (Statement wrap_stmt : wrap_stmts) {
                Tools.replaceSymbolIn(wrap_stmt, index.getSymbol(), ub);
            }
