...
public class LoopProfiler extends TransformPass {
    ...
    // and target position.
    private static void insertTiming(String name, Statement ref, int id, int where) {
        FunctionCall fc = new FunctionCall(new Identifier(name));
        ...
        if (where == ADD) {
            ...
            Tools.printlnStatus("[WARNING] unknown profiling code being inserted", 0);
        }
    }
    ...
    private void addTimingLibrary() {
        ...
        if (main_proc == null) {
            ...
            Statement init_timer = new ExpressionStatement(
                    new FunctionCall(new Identifier("cetus_init_timer")));
            ...
            Statement print_timer = new ExpressionStatement(
                    new FunctionCall(new Identifier("cetus_print_timer")));
            addConditionalCode(print_timer);

            if (return_stmts.size() == 0) {
                insertToc(main_proc.getBody(), num_loops++, ADD);
                main_proc.getBody().addStatement((Statement) print_timer.clone());
            } else {
                for (Statement stmt : return_stmts) {
                    ...
                    parent.addStatementBefore(stmt, (Statement) print_timer.clone());
                }
                num_loops++;
            }
        }
