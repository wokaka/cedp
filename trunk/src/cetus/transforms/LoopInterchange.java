 ...
 */
public class LoopInterchange extends AnalysisPass
{
//  protected Program program;
  ...
  public void start()
  {
				...
				for(i = outer_loops.size()-1; i >= 0; i--)
				{
						...
						if(loops.size() < 2) {
								num_single++;
						}	else if(!LoopTools.isPerfectNest(loops.get(0))) {
								num_non_perfect++;
						}	else if(LoopTools.containsFunctionCall(loops.get(0))) {
								num_contain_func++;
						} else {
								target_loops++;
						 ...
						 int r = 0,j,until = loops.size();
							...
							while(icFlag)
							{
									...
									for(j = 0; j < until; j++)
								 {
											exp = LoopTools.getIndexVariable(loops.get(j));
											if(exp != null)
													expList.add(exp);
									}
  ...
  // Find out which loops could legally interchanged with innermost loop.
		private List<Integer> rankByMaxInterchange(List<Integer> rank, List<Loop> loops)
		{
		...
		private List<Integer> rankByNumOfIteration(List<Integer> rank, List<Loop> loops)
		{
				@SuppressWarnings("unused")
				int i, rankSize;
				boolean flag = true;
		...
		@SuppressWarnings("unused")
		private int getRank2(List<Integer> rank, List<Expression> expList, List<Loop> loops)
		{
				//int i;
				List<Integer> result;
  ...
  private List<Integer> getRank(List<ArrayAccess> array , List<Expression> expList, int n)
		{
