...
public class LoopNormalization extends TransformPass
{
  ...
  public String getPassName()
  {
	...
	{
		if ( Tools.containsClasses(e, avoid_set) )
			return false;
		List<Specifier> type_list = Tools.getExpressionType(e);
		return (
			type_list != null &&
			type_list.size() == 1 &&
	...
	private void normalizeLoop(ForLoop loop)
	{
		...
		Tools.printStatus(getPassName()+" "+loop.getProcedure().getName()+": ",1);
		Tools.printStatus(LoopTools.toControlString(loop)+" --> ", 1);
		...
		*/
		...
		Expression nzt_condition = (Expression)loop.getCondition().clone();

		// Create a temporary index.
		Identifier new_index = Tools.getTemp(loop.getParent(), index);
		...
		Tools.printlnStatus(LoopTools.toControlString(loop), 1);

		// Modify the original index variable in terms of the new index variable.
		Tools.replaceSymbolIn(loop.getBody(), index.getSymbol(), subst);
