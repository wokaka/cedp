 ...
 * <pre>
 * Map range_map = getRangeMap(proc);
...
public class RangeDomain implements Cloneable
{
	...
	public void removeRangeWith(Symbol var)
	{
		...
		for ( Symbol symbol : symbols )
			if ( symbol == var || Tools.containsSymbol(getRange(symbol), var) )
   ...
   */
	public Expression toExpression()
	{
		...
		for ( Identifier var : ordered.keySet() )
		{
			...
			if ( range instanceof RangeExpression )
			{
				Expression lb = (Expression)((RangeExpression)range).getLB().clone();
				Expression ub = (Expression)((RangeExpression)range).getUB().clone();
	...
	public Relation compare(Expression e1, Expression e2)
	{
		...
		{
			Tools.printlnStatus(tag+e1+" "+ret+" "+e2+" under "+this, 5);
			return ret;
		}
	...
	(Expression e1, RangeDomain rd1, Expression e2, RangeDomain rd2)
	{
		...
		Tools.printlnStatus(tag+e1+" "+ret+" "+e2+" under "+rd1+" and "+rd2, 5);
		return ret;
	}
...
*/
	...
	{
		Set<Class<? extends Traversable>> avoid_set = new HashSet<Class<? extends Traversable>>();
			...
			while ( iter.hasNext() )
			{
				...
				if ( o instanceof ArrayAccess )
					for ( Expression e : ((ArrayAccess)o).getIndices() )
						if ( Tools.containsSymbol(e, symbol) )
						{
							removeRange(var);
							break;
						}
   ...
   */
			...
			expandMonoSubexprs(Expression e, Symbol var, RangeExpression range)
	{
		...
		else if ( isNonDecreasingWith(e, var) )
		{
			if ( range.getLB() instanceof InfExpression )
				...
				lb = Symbolic.simplify(Tools.replaceSymbol(e, var, range.getLB()));
			if ( range.getUB() instanceof InfExpression )
				ub = new InfExpression(1);
			else
				ub = Symbolic.simplify(Tools.replaceSymbol(e, var, range.getUB()));
		...
		else if ( isNonIncreasingWith(e, var) )
		{
			if ( range.getUB() instanceof InfExpression )
				...
				lb = Symbolic.simplify(Tools.replaceSymbol(e, var, range.getUB()));
			if ( range.getLB() instanceof InfExpression )
				ub = new InfExpression(1);
			else
				ub = Symbolic.simplify(Tools.replaceSymbol(e, var, range.getLB()));
			ret = new RangeExpression(lb, ub);
		}
	...
	public Expression replaceSymbol(Expression e, Symbol var, Expression with)
	{
		...
		//Tools.print("[RangeDomain.replaceSymbol] ", 9);
		//Tools.println("e=" + e.toString() + " var=" + var.toString() + " with=" + with.toString(), 9);
	...
	protected void expandSymbol(Symbol var)
	{
		...
		RangeDomain clone = (RangeDomain)this.clone();
 ...
 */
	public Expression expandSymbols(Expression e, Set<Symbol> vars)
	{
		...
		//Tools.print("[RangeDomain.expandSymbols] e=" + e, 9);
		//Tools.println(" list=" + Tools.collectionToString(order, ","),9);
 ...
 */
	...
	private boolean isNonDecreasingWith(Expression e, Symbol var)
	{
		...
		Expression next = Tools.replaceSymbol(e, var, Symbolic.increment(index));
	...
	private List<Symbol> getReplaceOrder(Expression e)
	{
		...
		List<Object> scc = rdg.getSCC(root);

		for ( Object o: scc )
		{
			@SuppressWarnings("unchecked")
			List<Object> tree = (List<Object>)o;
	...
	private Expression expandADD(Expression e1, Expression e2)
	{
		...
		return new RangeExpression((Expression)lb.clone(), (Expression)ub.clone());
	...
	public void intersectRanges(RangeDomain other)
	{
		...
		Tools.printlnStatus(dmsg+" = "+this, 5);
	}
	...
	(Expression e1, RangeDomain rd1, Expression e2, RangeDomain rd2)
	{
		...
		RangeExpression range = new RangeExpression(
			(Expression)lb.clone(), (Expression)ub.clone());
    ...
    ====================================================================*/
	@SuppressWarnings("unused")
	private static String SymbolstoString(Collection<Traversable> c)
	{
		String ret = "{";
	...
	private static boolean isDiscarded(Expression e)
	{
		...
		Set<Class<? extends Traversable>> discard_set = new HashSet<Class<? extends Traversable>>();
	...
	{
		List<Traversable> child1 = e1.getChildren(), child2 = e2.getChildren();
