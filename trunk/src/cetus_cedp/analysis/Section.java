...
public class Section extends ArrayList<Section.ELEMENT> implements Cloneable
{
	...
	{
		Tools.println("  [Section.expandMay] vars = " + Tools.collectionToString(vars, ","), 5);
		Tools.println("                      rd = " + rd.toString(), 5);
		for ( ELEMENT elem : this )
			for ( int i=0; i<dimension; ++i )
			{
				Tools.println("  [Section.expandMay] section = " + elem.get(i).toString(), 5);

				Expression expanded = rd.expandSymbols(elem.get(i), vars);
				Tools.println("  [Section.expandMay] expanded = " + expanded.toString(), 5);
				elem.set(i, expanded);
			}
		simplifyMe();
	}
	...
	public void substituteForward(RangeDomain rd, Set<Symbol> avoid)
	{
		for ( ELEMENT elem : this )
		{
			for ( int i=0; i<dimension; ++i )
			{
				...
				if ( !Tools.containsSymbols(substituted, avoid) )
	...
	public Section intersectWith(Section other, RangeDomain rd)
	{
		...
		Tools.printlnStatus(this+" (^) "+other+" = "+ret+" under "+rd, 5);
	...
	public Section unionWith(Section other, RangeDomain rd)
	{
		...
		Tools.printlnStatus(this+" (v) "+other+ " = "+ret+" under "+rd, 5);
	...
	public Section differenceFrom(Section other, RangeDomain rd)
	{
		...
		if ( dimension != other.dimension )
		{
			Tools.printlnStatus(mesg+ret, 5);
			return ret;
		}
		...
		Tools.printlnStatus(mesg+ret+" under "+rd, 5);
			...
			while ( !temp.isEmpty() )
			{
				...
				while ( iter.hasNext() )
				{
					...
					if ( elem2.isAdjacentWest(elem1, i) )
					{
						...
						elem1.set(i, new RangeExpression(re2.getLB(), re1.getUB()));
						iter.remove();
					}
					else if ( elem2.isAdjacentEast(elem1, i) )
					{
						RangeExpression re1 = RangeExpression.toRange(elem1.get(i));
						RangeExpression re2 = RangeExpression.toRange(elem2.get(i));
						elem1.set(i, new RangeExpression(re1.getLB(), re2.getUB()));
						iter.remove();
					}
				}
				ret.add(elem1);
	...
	public static class MAP extends HashMap<Symbol,Section> implements Cloneable
	{
		...
		public MAP differenceFrom2(MAP other, RangeDomain rd)
		{
			...
			for ( Symbol var : vars )
			{
				...
				{
					RangeDomain modified = (RangeDomain)rd.clone();
		...
		{
			if (isEmpty())
				Tools.println(str + " is empty", verbosity);
			else
			{
				if ( keySet() == null )
					Tools.println(str + " keySet() is null", verbosity);
				else if ( keySet().size() == 0 )
					Tools.println(str + " keySet() is empty", verbosity);
				else
				{
					Tools.print(str + " = [", verbosity);
					int count=0;
					for ( Symbol symbol : keySet() )
					{
						if (++count > 1) Tools.print(", ", verbosity);
						Section section = get(symbol);
						if (section.getDimension() < 1)		// scalar variable
							Tools.print(symbol.getSymbolName(), verbosity);	
						else
							Tools.print(symbol.getSymbolName() + "=" + section.toString(), verbosity);	
					}
					Tools.println("]", verbosity);
				}
			}
		}
