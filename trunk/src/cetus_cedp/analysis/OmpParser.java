...
public class OmpParser
{
	...
	private	static HashMap<String, Object> omp_map;
	...
	public static boolean parse_omp_pragma(HashMap<String, Object> input_map, String [] str_array)
	{
		omp_map = input_map;
...
//		default : throw new NonOmpDirectiveException();
	...
	{
		Tools.println("OmpParser is parsing [parallel] clause", 2);
	...
	{
		Tools.println("OmpParser is parsing [for] clause", 2);
		addToMap("for", "true");
		while (end_of_token() == false) 
		{
			String clause = "token_" + get_token();
	...
	{
		Tools.println("OmpParser is parsing [sections] clause", 2);
	...
	{
		Tools.println("OmpParser is parsing [section] clause", 2);
	...
	{
		Tools.println("OmpParser is parsing [single] clause", 2);
	...
	{
		Tools.println("OmpParser is parsing [parallel for] clause", 2);
		addToMap("for", "true");
		while (end_of_token() == false) 
		{
			String clause = "token_" + get_token();
	...
	{
		Tools.println("OmpParser is parsing [parallel sections] clause", 2);
		while (end_of_token() == false) 
		{
			String clause = "token_" + get_token();
	...
	{
		Tools.println("OmpParser is parsing [task] clause", 2);
		while (end_of_token() == false) 
		{
			String clause = "token_" + get_token();
			addToMap("task", "true");
	...
	{
		Tools.println("OmpParser is parsing [master] clause", 2);
		addToMap("master", "true");
	}

	private static void parse_omp_critical()
	{
		Tools.println("OmpParser is parsing [critical] clause", 2);
		String name = null;
	...
	{
		Tools.println("OmpParser is parsing [barrier] clause", 2);
		addToMap("barrier", "true");
	}

	private static void parse_omp_taskwait()
	{
		Tools.println("OmpParser is parsing [taskwait] clause", 2);
		addToMap("taskwait", "true");
	}

	private static void parse_omp_atomic()
	{
		Tools.println("OmpParser is parsing [atomic] clause", 2);
		addToMap("atomic", "true");
	}

	private static void parse_omp_flush()
	{
		Tools.println("OmpParser is parsing [flush] clause", 2);
	...
	{
		Tools.println("OmpParser is parsing [ordered] clause", 2);
...
/**
  ...
  that every parallel region has to be annotated as private(x)
	*/
	...
	{
		Tools.println("OmpParser is parsing [threadprivate] clause", 2);
		match("(");
		Set<String> set = new HashSet<String>();
		parse_commaSeparatedList(set);
		match(")");
	...
	{
		Tools.println("OmpParser is parsing [if] clause", 2);
	...
	{
		Tools.println("OmpParser is parsing [omp_num_threads]", 2);
		String str = parse_ParenEnclosedExpr();
	...
	{
		Tools.println("OmpParser is parsing [schedule] clause", 2);
		String str = null;
	...
	{
		Tools.println("OmpParser is parsing [collapse] clause", 2);
		match("(");
	...
	private static void parse_omp_nowait() { 
		Tools.println("OmpParser is parsing [nowait] clause", 2);
		addToMap("nowait", "true"); 
	}

	private static void parse_omp_untied() { 
		Tools.println("OmpParser is parsing [untied] clause", 2);
		addToMap("untied", "true"); 
	}

	private static void parse_omp_default()
	{
		Tools.println("OmpParser is parsing [default] clause", 2);
		match("(");
	...
	{
		Tools.println("OmpParser is parsing [private] clause", 2);
		match("(");
		Set<String> set = new HashSet<String>();
		parse_commaSeparatedList(set);
		match(")");
	...
	{
		Tools.println("OmpParser is parsing [firstprivate] clause", 2);
		match("(");
		Set<String> set = new HashSet<String>();
		parse_commaSeparatedList(set);
		match(")");
	...
	{
		Tools.println("OmpParser is parsing [lastprivate] clause", 2);
		match("(");
		Set<String> set = new HashSet<String>();
		parse_commaSeparatedList(set);
		match(")");
	...
	{
		Tools.println("OmpParser is parsing [copyprivate] clause", 2);
		match("(");
		Set<String> set = new HashSet<String>();
		parse_commaSeparatedList(set);
		match(")");
	...
	{
		Tools.println("OmpParser is parsing [shared] clause", 2);
		match("(");
		Set<String> set = new HashSet<String>();
		parse_commaSeparatedList(set);
		match(")");
	...
	{
		Tools.println("OmpParser is parsing [copyin] clause", 2);
		match("(");
		Set<String> set = new HashSet<String>();
		parse_commaSeparatedList(set);
		match(")");
	...
	{
		Tools.println("OmpParser is parsing [reduction] clause", 2);
		HashMap<String, Set<String>> reduction_map = null;
		Set<String> set = null;
		String op = null;
		...
		{
			op = get_token();
			Tools.println("reduction op:" + op, 2);
		}
		else {
		...
		for (String ikey : omp_map.keySet())
		{
			if (ikey.compareTo("reduction") == 0)
			{
				reduction_map = (HashMap<String, Set<String>>)omp_map.get(ikey);
				set = reduction_map.get(op);
				break;
			}
		}

		if (reduction_map == null) 
			{ reduction_map = new HashMap<String, Set<String>>(4); } 
	...
	private static void addToMap(String key, Set<String> new_set)
	{
		if (omp_map.keySet().contains(key))
		{
			Set<String> set = (Set<String>)omp_map.get(key);
			set.addAll(new_set);
		}
		else
		{
	...
	@SuppressWarnings("unchecked")
	private static void addToMap(String key, HashMap<String, Set<String>> new_map)
	{
		if (omp_map.keySet().contains(key))
		{
			Map<String, Set<String>> orig_map = (Map<String, Set<String>>)omp_map.get(key);
