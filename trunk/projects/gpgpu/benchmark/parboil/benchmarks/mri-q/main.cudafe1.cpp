...
__loc_sc__(__constant__,,) kValues __shadow_var(,ck)[1024]; 
...
# 30
{ 
auto int numK_dup = numK; 

auto float *phiR_dup = phiR; 
auto float *phiI_dup = phiI; 
auto float *phiMag_dup = phiMag; 

auto int indexK = (blockIdx.x * (512) + threadIdx.x); 
auto int indexK_dup = (blockIdx.x * (512) + threadIdx.x); 

{ if (indexK != indexK_dup) { GPUFI_EXIT(0); }  } ; 
{ if (numK != numK_dup) { GPUFI_EXIT(0); }  } ; 
if (indexK < numK) 
{ 

if (phiR != phiR_dup) { GPUFI_EXIT(0); }  
{ if (indexK != indexK_dup) { GPUFI_EXIT(0); }  } ; 
auto float real = (phiR[indexK]); 
auto float real_dup = (phiR[indexK]); 


if (phiI != phiI_dup) { GPUFI_EXIT(0); }  
{ if (indexK != indexK_dup) { GPUFI_EXIT(0); }  } ; 
auto float imag = (phiI[indexK]); 
auto float imag_dup = (phiI[indexK]); 


if (phiMag != phiMag_dup) { GPUFI_EXIT(0); }  
{ if (indexK != indexK_dup) { GPUFI_EXIT(0); }  } ; 
{ if (real != real_dup) { GPUFI_EXIT(0); }  } ; 
{ if (imag != imag_dup) { GPUFI_EXIT(0); }  } ; 
(phiMag[indexK]) = real * real + imag * imag; 
{ if ((phiMag[indexK]) != (real * real + imag * imag)) { GPUFI_EXIT(0); }  } ; 
}  
} 
#endif
...
{ 
auto int numK_dup = numK; 
auto int kGlobalIndex_dup = kGlobalIndex; 

auto float *x_dup = x; 
auto float *y_dup = y; 
auto float *z_dup = z; 
auto float *Qr_dup = Qr; 
auto float *Qi_dup = Qi; 
...
auto int xIndex_dup = (blockIdx.x * (256) + threadIdx.x); 
# 93 "src/cuda_ed5/computeQ.cu"
auto int kIndex = 0; 
auto int kIndex_dup = 0; 


if (x != x_dup) { GPUFI_EXIT(0); }  
{ if (xIndex != xIndex_dup) { GPUFI_EXIT(0); }  } ; 
sX = x[xIndex]; 
auto float sX_dup = (x[xIndex]); 


if (y != y_dup) { GPUFI_EXIT(0); }  
{ if (xIndex != xIndex_dup) { GPUFI_EXIT(0); }  } ; 
sY = y[xIndex]; 
auto float sY_dup = (y[xIndex]); 


if (z != z_dup) { GPUFI_EXIT(0); }  
{ if (xIndex != xIndex_dup) { GPUFI_EXIT(0); }  } ; 
sZ = z[xIndex]; 
auto float sZ_dup = (z[xIndex]); 


if (Qr != Qr_dup) { GPUFI_EXIT(0); }  
{ if (xIndex != xIndex_dup) { GPUFI_EXIT(0); }  } ; 
sQr = Qr[xIndex]; 
auto float sQr_dup = (Qr[xIndex]); 


if (Qi != Qi_dup) { GPUFI_EXIT(0); }  
{ if (xIndex != xIndex_dup) { GPUFI_EXIT(0); }  } ; 
sQi = Qi[xIndex]; 
auto float sQi_dup = (Qi[xIndex]); 
...
{ if (sX != sX_dup) { GPUFI_EXIT(0); }  } ; 
{ if (sY != sY_dup) { GPUFI_EXIT(0); }  } ; 
{ if (sZ != sZ_dup) { GPUFI_EXIT(0); }  } ; 
auto float expArg = ((6.283185307179586232) * ((((__shadow_var(,ck))[0]).Kx * sX + ((__shadow_var(,ck))[0]).Ky * sY) + ((__shadow_var(,ck))[0]).Kz * sZ)); 
auto float expArg_dup = ((6.283185307179586232) * ((((__shadow_var(,ck))[0]).Kx * sX + ((__shadow_var(,ck))[0]).Ky * sY) + ((__shadow_var(,ck))[0]).Kz * sZ)); 

{ if (expArg != expArg_dup) { GPUFI_EXIT(0); }  } ; 
sQr += ((__shadow_var(,ck))[0]).PhiMag * cos(expArg); 

{ if (expArg != expArg_dup) { GPUFI_EXIT(0); }  } ; 
sQi += ((__shadow_var(,ck))[0]).PhiMag * sin(expArg); 
{ if (kIndex != kIndex_dup) { GPUFI_EXIT(0); }  } ; 
kIndex++; 
kIndex_dup++; 
{ if (kGlobalIndex != kGlobalIndex_dup) { GPUFI_EXIT(0); }  } ; 
kGlobalIndex++; 
kGlobalIndex_dup++; 
}  
...
{ if (sY != sY_dup) { GPUFI_EXIT(0); }  } ; 
{ if (sZ != sZ_dup) { GPUFI_EXIT(0); }  } ; 
auto float expArg = ((6.283185307179586232) * ((((__shadow_var(,ck))[kIndex]).Kx * sX + ((__shadow_var(,ck))[kIndex]).Ky * sY) + ((__shadow_var(,ck))[kIndex]).Kz * sZ)); 
auto float expArg_dup = ((6.283185307179586232) * ((((__shadow_var(,ck))[kIndex]).Kx * sX + ((__shadow_var(,ck))[kIndex]).Ky * sY) + ((__shadow_var(,ck))[kIndex]).Kz * sZ)); 
auto int kIndex1; 
...
{ if (kIndex != kIndex_dup) { GPUFI_EXIT(0); }  } ; 
{ if (expArg != expArg_dup) { GPUFI_EXIT(0); }  } ; 
sQr += ((__shadow_var(,ck))[kIndex]).PhiMag * cos(expArg); 
sQr_dup += ((__shadow_var(,ck))[kIndex]).PhiMag * cos(expArg); 

{ if (kIndex != kIndex_dup) { GPUFI_EXIT(0); }  } ; 
{ if (expArg != expArg_dup) { GPUFI_EXIT(0); }  } ; 
sQi += ((__shadow_var(,ck))[kIndex]).PhiMag * sin(expArg); 
{ if (expArg != expArg_dup) { GPUFI_EXIT(0); }  } ; 
sQi_dup += ((__shadow_var(,ck))[kIndex]).PhiMag * sin(expArg); 
{ if (kIndex != kIndex_dup) { GPUFI_EXIT(0); }  } ; 
kIndex1_dup = kIndex + 1; 
kIndex1 = kIndex + 1; 

{ if (kIndex1 != kIndex1_dup) { GPUFI_EXIT(0); }  } ; 
{ if (sX != sX_dup) { GPUFI_EXIT(0); }  } ; 
{ if (sY != sY_dup) { GPUFI_EXIT(0); }  } ; 
{ if (sZ != sZ_dup) { GPUFI_EXIT(0); }  } ; 
expArg1 = (6.283185307179586232) * ((((__shadow_var(,ck))[kIndex1]).Kx * sX + ((__shadow_var(,ck))[kIndex1]).Ky * sY) + ((__shadow_var(,ck))[kIndex1]).Kz * sZ); 
expArg1_dup = (6.283185307179586232) * ((((__shadow_var(,ck))[kIndex1]).Kx * sX + ((__shadow_var(,ck))[kIndex1]).Ky * sY) + ((__shadow_var(,ck))[kIndex1]).Kz * sZ); 

{ if (kIndex1 != kIndex1_dup) { GPUFI_EXIT(0); }  } ; 
{ if (expArg != expArg_dup) { GPUFI_EXIT(0); }  } ; 
sQr += ((__shadow_var(,ck))[kIndex1]).PhiMag * cos(expArg1); 
{ if (expArg != expArg_dup) { GPUFI_EXIT(0); }  } ; 
sQr_dup += ((__shadow_var(,ck))[kIndex1]).PhiMag * cos(expArg1); 

{ if (kIndex1 != kIndex1_dup) { GPUFI_EXIT(0); }  } ; 
{ if (expArg != expArg_dup) { GPUFI_EXIT(0); }  } ; 
sQi += ((__shadow_var(,ck))[kIndex1]).PhiMag * sin(expArg1); 
{ if (expArg != expArg_dup) { GPUFI_EXIT(0); }  } ; 
sQi_dup += ((__shadow_var(,ck))[kIndex1]).PhiMag * sin(expArg1); 
}  

if (Qr != Qr_dup) { GPUFI_EXIT(0); }  
{ if (xIndex != xIndex_dup) { GPUFI_EXIT(0); }  } ; 
(Qr[xIndex]) = sQr; 
{ if ((Qr[xIndex]) != sQr) { GPUFI_EXIT(0); }  } ; 


if (Qi != Qi_dup) { GPUFI_EXIT(0); }  
{ if (xIndex != xIndex_dup) { GPUFI_EXIT(0); }  } ; 
(Qi[xIndex]) = sQi; 
{ if ((Qi[xIndex]) != sQi) { GPUFI_EXIT(0); }  } ; 
} 
#endif
...
cudaMemcpyToSymbol(__shadow_var(,ck), kValsTile, numElems * sizeof(kValues), 0); 
