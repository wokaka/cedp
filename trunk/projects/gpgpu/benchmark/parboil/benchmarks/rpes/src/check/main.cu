...
__device__ float *d_val_fp;
__device__ float *d_err;
...
int main(int argc, char* argv[])
{
  ...
  // prepare device data for FI
  cudaMalloc((void**)&d_val, 4*10000*32);
  CUDA_ERRCK
  cudaMalloc((void**)&d_val_fp, 4*10000*32);
  CUDA_ERRCK
  cudaMalloc((void**)&d_err, 4*10000*32);
  CUDA_ERRCK
  ...
  /* generate random number */
      ...
      //NumInteger <<< grid, block >>> (d_val, d_err, 10000);
      
      NumFloat_Gen <<< grid, block >>> (d_val_fp, d_err, 10000);
      CUDA_ERRCK

    }

	float *h_val_fp;
	h_val_fp = (float *)malloc(4*10000*32);
  cudaMemcpy(h_val_fp, d_val_fp, 4*10000*32, cudaMemcpyDeviceToHost);
  CUDA_ERRCK
	...
	h_val = (int *)malloc(4*10000*32);
	h_err = (float *)malloc(4*10000*32);
  ...
  cudaMemcpy(h_err, d_err, 4*10000*32, cudaMemcpyDeviceToHost);
  CUDA_ERRCK
  cudaFree(d_err);
  CUDA_ERRCK
  ...
  if(params->outFile)
    {
      ...
      for(int i = 0; i < 10000; i ++)
        {
          //fprintf(file, "%d\t", (int)h_val[i*32+j]);
          fprintf(file, "%e\t", (int)h_val_fp[i*32+j]);
        	for(j=0; j<32; j++){
          	fprintf(file, "%e\t", (float)h_err[i*32+j]);
        	}
        	fprintf(file, "\n");
        }
      fclose(file);
  ...
  for(int run = 0; run < runs; run ++)
    {
      ...
      DoReduction <<< grid, block >>> (d_ReductionSum, d_Output, 
				       d_FinalReduce, MaxBlocks, Offset, gpufi_dev);
      CUDA_ERRCK
