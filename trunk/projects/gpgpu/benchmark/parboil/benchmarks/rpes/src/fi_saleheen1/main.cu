...
struct gpufi_profile gpufi_main_profile = {0, };
struct gpufi_current gpufi_main_current = {0, };
struct gpufi_fault gpufi_main_fault;
...
// allocate data in device for FI
	
	struct gpufi_fault *ptr_gpufi_main_fault;
	ptr_gpufi_main_fault = &gpufi_main_fault;

	struct gpufi_fault *d_gpufi_fault;
	cudaMalloc((void**)&d_gpufi_fault, sizeof(struct gpufi_fault));
	cudaMemcpy(d_gpufi_fault, ptr_gpufi_main_fault, sizeof(struct gpufi_fault), cudaMemcpyHostToDevice);

	struct gpufi_profile *ptr_gpufi_main_profile;
	ptr_gpufi_main_profile = &gpufi_main_profile;

	struct gpufi_profile *d_gpufi_profile;
	cudaMalloc((void**)&d_gpufi_profile, sizeof(struct gpufi_profile));
	cudaMemcpy(d_gpufi_profile, ptr_gpufi_main_profile, sizeof(struct gpufi_profile), cudaMemcpyHostToDevice);

	struct gpufi_current *ptr_gpufi_main_current;
	ptr_gpufi_main_current = &gpufi_main_current;

	struct gpufi_current *d_gpufi_current;
	cudaMalloc((void**)&d_gpufi_current, sizeof(struct gpufi_current));
	cudaMemcpy(d_gpufi_current, ptr_gpufi_main_current, sizeof(struct gpufi_current), cudaMemcpyHostToDevice);
  ...
  for(int run = 0; run < runs; run ++)
    {
      ...
      ComputeX <<< grid, block >>> (d_Block_Work, d_Output, StartBlock,d_gpufi_fault, d_gpufi_profile, d_gpufi_current);
      CUDA_ERRCK
...
//  get FI data back from device
	cudaMemcpy(ptr_gpufi_main_fault, d_gpufi_fault, sizeof(struct gpufi_fault), cudaMemcpyDeviceToHost);
	cudaMemcpy(ptr_gpufi_main_profile, d_gpufi_profile, sizeof(struct gpufi_profile), cudaMemcpyDeviceToHost);
	cudaMemcpy(ptr_gpufi_main_current, d_gpufi_current, sizeof(struct gpufi_current), cudaMemcpyDeviceToHost);

	cudaFree(d_gpufi_fault);
	cudaFree(d_gpufi_profile);
	cudaFree(d_gpufi_current);
...
int GPUFI_FETCH_COMMAND()
{
    ...
    if(!strcmp(cmd, "profile")){
	   gpufi_main_fault.mode = GPUFI_PROFILE;
    }
    else if(!strcmp(cmd, "fi")){
	   gpufi_main_fault.mode = GPUFI_FI;
	    fscanf(fp, "%d %d %d %d %s %d\n", &gpufi_main_fault.kernel, &gpufi_main_fault.instance,
		&gpufi_main_fault.loop, &gpufi_main_fault.iteration,
		gpufi_main_fault.variable_name, &gpufi_main_fault.mask);
	    gpufi_main_fault.kernel_count = gpufi_main_fault.kernel;
	    gpufi_main_fault.mask_type = MASK_XOR;
	    gpufi_main_fault.injected = 0;
    }
...
int GPUFI_END()
{
    ...
    if(gpufi_main_fault.mode == GPUFI_PROFILE){
        //char    cmd[32];
        ...
        fprintf(fp, "kernel\t%d\n", gpufi_main_profile.kernels);
        for(i=0; i<gpufi_main_profile.kernels; i++){
            fprintf(fp, "kernel#\t%d\n", i);
            fprintf(fp, "instance\t%d\n", gpufi_main_profile.instance[i]);

            for(j=0; j<gpufi_main_profile.instance[i]; j++){
                fprintf(fp, "instance#\t%d\n", j);
                fprintf(fp, "loop\t%d\n", gpufi_main_profile.loop[i][j]);

                for(k=0; k<gpufi_main_profile.loop[i][j]; k++){
                    fprintf(fp, "loop#\t%d\n", j);
                    fprintf(fp, "iteration\t%d\n", gpufi_main_profile.iteration[i][j][k]);

                    for(l=0; l<MAX_VARIABLE; l++){
                        fprintf(fp, "variable\t%d\n", gpufi_main_profile.variable[i][j][k][l]);
                    }
                }
            }
        }
