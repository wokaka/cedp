  ...
  if(params->outFile)
    {
      ...
      for(int i = 0; i < reduction_mem / sizeof(float)/*20000*/; i ++)
        {
          fprintf(file, "%d\t%e\n", i, ReductionSum[i]);
        }
      fclose(file);
