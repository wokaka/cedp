...
public class JcraftWrapper {
    ...
    public JcraftWrapper(final String address, final int port, final String id, final String pwd, JTextArea area)
    {
        is = new JcraftInputStream();
        os = new JcraftOutputStream(area);

        new UtilThread(null){public void run(){Connect(address, port, id, pwd);}};
...
//            session.setConfig(config);
    ...
    public class JcraftOutputStream extends OutputStream {
        ...
        {
            if(pos != -1){
                System.out.println("Jcraft: retry after when the previous thread completes");
                return;
            }
