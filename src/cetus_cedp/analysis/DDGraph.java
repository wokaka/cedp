...
public class DDGraph 
{
	...
	static public class Arc
	{
		...
		public boolean isCarried(Loop l)
		{
			...
			DependenceVector dv = this.getDirectionVector();
			
			if ((dv.vector).containsKey(l))
			{
	...
	public void addArc(Arc arc_to_add)
	{
		...
		if (arc_to_add.getDirectionVector().isValid() == true) {
			for (Arc arc : depArcs) {
				if ((arc.getSource() == arc_to_add.getSource()) &&
					(arc.getSink() == arc_to_add.getSink()) &&
					(arc.getDependenceType() == arc_to_add.getDependenceType()) &&
					((arc.getDirectionVector().vector).equals(
						arc_to_add.getDirectionVector().vector)))
...
//			Tools.print("Stuck in filterUnwantedArcs", 0);
			if ( !dv.isValid() )
				//this.deleteArc(arc);
				...
				while (arc_list_2.hasNext())
				{
					...
					if ( arc2.getDirectionVector().isValid() )
					{
						if ((arc1.getSource() == arc2.getSource()) &&
							...
							arc2.getDirectionVector().setValid(false);
					}
				}
			}
		}
	...
	public void summarizeGraph()
	{
		...
		while (arc_list.hasNext())
		{
			...
			if ( arc.getDirectionVector().isValid() )
			{
				...
				for (Loop l : loopsInVector)
				{
							...
							(great_dir_cnt > 0))
					{
						for (Arc a :  arcsToBeSummarized)
						{
							...
							a.getDirectionVector().setValid(false);
						}
					}
				}
			}
	...
	{
		return (checkDependenceType(e1, e2, (byte)1));
	}
	...
	{
		return (checkDependenceType(e1, e2, (byte)2));
	}
	...
	{
		return (checkDependenceType(e1, e2, (byte)3));
	}
	...
	public ArrayList<DependenceVector> getDirectionMatrix(LinkedList<Loop> nest)
	{
		...
		for (Arc dep : depArcs)
		{
			DependenceVector dv = dep.getDirectionVector();
