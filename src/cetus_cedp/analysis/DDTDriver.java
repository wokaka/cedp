...
public class DDTDriver extends AnalysisPass
{
	...
	private Map<Procedure, Map<Statement, RangeDomain>> proc_range_map;
	...
	public void start()
	{
		...
		// information here
		proc_range_map = new HashMap<Procedure, Map<Statement, RangeDomain>>();
		...
		Tools.println("Number of eligible outermost loops for this program = " + eligible_loops.size(), 1);
		...
		Tools.println("Analysis time for data dependence testing: " + elapsed_time, 1);
		Tools.println("Size of data dependence graph = " + (ddg.getAllArcs()).size(), 1);
	}
			...
			Traversable root)
	{
		...
		if (root instanceof Expression)
		{
			...
			HashSet<Class<? extends Traversable>> of_interest = new HashSet<Class<? extends Traversable>>();
...
*/			}
			...
			HashMap<Loop, LoopInfo> loopInfoMap)
	{
		...
		for (Symbol name: arrayNames)
		{
			 ...
			 * ALIAS SET CHECK
			 */
			Set<Symbol> alias_set = alias_analysis.get_alias_set((Statement)loop, name);
			...
			while(iter_expr1.hasNext())
			{
				...
				if (expr1_info.getAccessType() == DDArrayAccessInfo.write_type)
				{
					...
					while (iter_expr2.hasNext())
					{
						...
						Symbol s1 = Tools.getSymbolOf(expr1_info.getArrayAccess());
						Symbol s2 = Tools.getSymbolOf(expr2_info.getArrayAccess());
						if (s1 == s2)
						{
							//---------------------------------------------------------------------------
							...
							Expression e1, e2;
							Set<Symbol> expr1_symbols = Tools.getAccessedSymbols(expr1_info.getArrayAccess());
							expr1_symbols.remove(Tools.getSymbolOf(expr1_info.getArrayAccessName()));
							Set<Symbol> expr2_symbols = Tools.getAccessedSymbols(expr2_info.getArrayAccess());
							expr2_symbols.remove(Tools.getSymbolOf(expr2_info.getArrayAccessName()));
	...
	private RangeDomain getStatementRangeDomain(Statement s)
	{
		...
		Map<Statement, RangeDomain> stmt_rmap;
		RangeDomain ret = null;
		Procedure stmt_proc = Tools.getParentProcedure(s);

		if (proc_range_map.containsKey(stmt_proc))
			stmt_rmap = proc_range_map.get(stmt_proc);
		else
		{
			stmt_rmap = RangeAnalysis.getRanges(stmt_proc);
