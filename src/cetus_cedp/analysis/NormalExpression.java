 ...
 * </pre>
 */
public class NormalExpression implements Comparable<Object>,Comparator<NormalExpression>
{
  // Type information
  ...
  private ArrayList<NormalExpression> list;  
...
//------------------------------------------------------------------------------
	...
	public static Expression getCoefficient(Expression e, Identifier id)
	{
		...
		{
			Iterator<NormalExpression> iter = ne.list.iterator();
			while ( iter.hasNext() )
			{
				Expression curr = (iter.next()).getExpression();
				if ( curr.equals(id) )
				{
					iter.remove();
					break;
	...
	(Expression e, List<Identifier> id_list)
	{
		List<Expression> ret = new ArrayList<Expression>(id_list.size()+1);
		for ( Identifier id : id_list )
  ...
  protected static List<Object> solveForVariables(Expression e)
  {
  ...
  private NormalExpression()
  {
    ...
    order = 0;
    list = new ArrayList<NormalExpression>();
    expr = null;
    value = null;
  }
  ...
  {
    return ( value instanceof FloatLiteral );
  }
  ...
  private void parseUnary(UnaryExpression e)
  {
    ...
    else if ( eop == UnaryOperator.MINUS )
    {
      if ( esub instanceof IntegerLiteral )
        value = new IntegerLiteral(-1*((IntegerLiteral)esub).getValue());
      else if ( esub instanceof FloatLiteral )
        value = new FloatLiteral(-1*((FloatLiteral)esub).getValue());
      else
      {
        op = OP.MUL;
...
//      list.add(e);
  ...
  private static boolean isNormal(Expression e)
  {
    ...
    {
      Set<String> op_set = new HashSet<String>(Arrays.asList(
        "+","-","*","/","==",">=",">","<=","<","!=","&&","||","%"));
  ...
  {
    Set<Traversable> unique_children = new TreeSet<Traversable>(); // TreeSet uses compareTo method.

    unique_children.addAll(e.getChildren());

    MinMaxExpression ret = new MinMaxExpression(e.isMin());

    TreeMap<Long, Traversable> literal_map = new TreeMap<Long, Traversable>();
  ...
  private NormalExpression foldADD()
  {
    ...
    // Fold numeric parts
    prev = null;
    List<NormalExpression> temp = ret.list;
    ret = new NormalExpression(OP.ADD);
    for ( NormalExpression curr : temp )
    {
      if ( curr.isLiteral() )
  ...
  private NormalExpression simplifyIntDiv()
  {
    ...
    NormalExpression ret = new NormalExpression(OP.MUL);
    List<NormalExpression> dividers = new ArrayList<NormalExpression>();
    ...
    for ( NormalExpression curr : dividers )
    {
