...
public class CCTreeWalker extends TreeWalker
{
  ...
  {
    super(filename);
    action_map = new HashMap<String,Integer>(200);
  ...
  protected IDExpression action_class_name(TreeNode root)
  {
    ...
    switch (root.getChildCount())
    {
      ...
      case 4:
        return new TemplateID(name, this.<List<Expression>>doAction(root.getChild(2)));
      default:
        throw new RuntimeException("bad class name");
    }
  }

  protected Expression action_condition(TreeNode root)
  {
    if (root.getChildCount() == 1)
      return this.<Expression>doAction(root.getChild(0));
    else
  ...
  protected Object action_declarator_tail(TreeNode root)
  {
    ...
    List<Declaration> params = this.<List<Declaration>>doAction(root.getChild(1));
...
try {
  ...
  throw e;
    ...
    if (root.getChild(1).getText().equals("("))
    {
      ...
      {
        Iterator<Declaration> iter = params.iterator();
        while (iter.hasNext())
          ((NestedDeclarator)direct_decl).addParameter(iter.next());
        return direct_decl;
      }
      else
        return new ProcedureDeclarator((IDExpression)((VariableDeclarator)direct_decl).getDirectDeclarator().clone(),
          this.<List<Declaration>>doAction(root.getChild(2)));
    }
  ...
  {
    return new TemplateDeclaration(new LinkedList<Declaration>(), 
					this.<Declaration>doAction(root.getChild(3)));
  }
  ...
  protected Procedure action_function_definition(TreeNode root)
  {
    ...
    if (child_count == 3)
      proc.setConstructorInitializers(this.<List<Expression>>doAction(root.getChild(1))); 
    return proc;
  }
  ...
  protected Procedure action_function_prefix(TreeNode root)
  {
    ...
    List<Specifier> specs = this.<List<Specifier>>doAction(root.getChild(0));
...
//    ProcedureDeclarator pdecl = null;
    ...
    if (child_count == 3)
    {
      List<?> list = this.<List<?>>doAction(root.getChild(2));
  ...
  protected Initializer action_initializer(TreeNode root)
  {
    if (root.getChildCount() == 2)
      ...
      return new ConstructorInitializer(this.<List<? extends Traversable>>doAction(root.getChild(1)));
  }
  ...
  protected Initializer action_initializer_clause(TreeNode root)
  {
    switch (root.getChildCount())
    {
      ...
      case 2:
        return new Initializer(new LinkedList<Traversable>());
      case 3:
      case 4:
  ...
  protected LinkageSpecification action_linkage_specification(TreeNode root)
  {
    ...
    switch (root.getChildCount())
    {
      ...
      case 4:
        return new LinkageSpecification(s, new LinkedList<Declaration>());

      case 5:
        return new LinkageSpecification(s, this.<List<Declaration>>doAction(root.getChild(3)));
  ...
  protected Declarator action_mem_initializer(TreeNode root)
  {
    ...
    if (root.getChildCount() == 4)
      decl.setInitializer(new ConstructorInitializer(this.<List<Traversable>>doAction(root.getChild(2))));
    else
      decl.setInitializer(new ConstructorInitializer(new LinkedList<Traversable>()));
    ...
    switch (root.getChildCount())
    {
      case 1:
        ...
        if (o != null)
          ...
          return new VariableDeclaration(new LinkedList<Specifier>());
      ...
      case 2:
        ...
        if (o instanceof List)
        {
          if (root.getChild(0).getText().startsWith("decl"))
            return new VariableDeclaration((List<Specifier>)o, new LinkedList<Declarator>());
          else
  ...
  protected List<Object> action_member_specification(TreeNode root)
  {
    int child_count = root.getChildCount();
    ...
    if (child_count == 3 ||
      ...
      list = this.<List<Object>>doAction(root.getChild(child_count - 1));
    else
  ...
  protected Namespace action_namespace_definition(TreeNode root)
  {
    ...
    if (root.getChild(child_count - 2).getText().equals("{"))
      decl_list = new LinkedList<Declaration>();
    else
    ...
    else if (o instanceof List)
    {
      ...
      for (Object o2 : list)
      {
        if (o2 instanceof List)
          specs.addAll((List<Specifier>)o2);
        else
  ...
  protected List<Declaration> action_parameter_declaration_clause(TreeNode root)
  {
    int child_count = root.getChildCount();
    ...
    switch (child_count)
    {
      ...
      case 1:
        if (root.getChild(0).getText().equals("..."))
          ...
          list = this.<List<Declaration>>doAction(root.getChild(0));
        return list;
  ...
  protected Expression action_postfix_expression(TreeNode root)
  {
    ...
    if (o == null)
    {
      ...
      if (s.endsWith("_cast"))
      {
        List<Specifier> specs = this.<List<Specifier>>doAction(root.getChild(2));
    ...
    else if (o instanceof Specifier)
    {
      if (root.getChildCount() == 3)
        return new Typecast(Typecast.NORMAL, (Specifier)o, new LinkedList<Expression>());
      else
        return new Typecast(Typecast.NORMAL, (Specifier)o, this.<List<Expression>>doAction(root.getChild(2)));
    }
  ...
  protected QualifiedID action_scoped_class_name(TreeNode root)
  {
    switch (root.getChildCount())
    {
      case 1:
        ...
        list.add(this.<IDExpression>doAction(root.getChild(0)));
        return new QualifiedID(list);

      case 3:
        QualifiedID qid = (QualifiedID)doAction(root.getChild(2));
        qid.getIDExpressionList().add(0, (IDExpression)doAction(root.getChild(0)));
        return qid;
  ...
  protected IDExpression action_scoped_unqualified_id(TreeNode root)
  {
    ...
    {
      List<IDExpression> list = new LinkedList<IDExpression>();
      list.add(class_name);
      list.add(second_id);
      return new QualifiedID(list);
    }
  ...
  protected Declaration action_simple_declaration(TreeNode root)
  {
    ...
    {
      case 1:
        return new VariableDeclaration(new LinkedList<Specifier>(), new LinkedList<Declarator>());
      case 2:
        List<?> list = this.<List<?>>doAction(root.getChild(0));
        if (list.get(0) instanceof Specifier)
          return new VariableDeclaration((List<Specifier>)list, new LinkedList<Declarator>());
        else
          return new VariableDeclaration(new LinkedList<Specifier>(), (List<Declarator>)list);
      case 3:
        return new VariableDeclaration(this.<List<Specifier>>doAction(root.getChild(0)),
          this.<List<Declarator>>doAction(root.getChild(1)));
      default:
  ...
  protected Specifier action_simple_type_specifier(TreeNode root)
  {
    ...
    if (root.getChild(1).getText().equals("("))
    {
      ...
      if (o instanceof Expression)
        ...
        return new TypeofSpecifier((List<Traversable>)o);
    }
    else
    {
      IDExpression expr = (IDExpression)doAction(root.getChild(1));
  ...
  protected Object action_template_argument(TreeNode root)
  {
    ...
    if (o instanceof List)
      o = new VariableDeclaration((List<Specifier>)o, new LinkedList<Declarator>());
  ...
  protected Object action_template_declaration(TreeNode root)
  {
    ...
    List<Declaration> params = this.<List<Declaration>>doAction(root.getChild(base + 2));
    Declaration decl = this.<Declaration>doAction(root.getChild(base + 4));
  ...
  protected List<Object> action_type_id(TreeNode root)
  {
...
/*
  ...
  protected Specifier action_type_specifier(TreeNode root)
  {
    ...
    if (s.equals("enum"))
    {
      ...
      if (child_count > 2)
      {
        List<Declarator> decl_list = this.<List<Declarator>>doAction(root.getChild(child_count - 2));
    ...
    try {
      List<Declaration> list = this.<List<Declaration>>doAction(root.getChild(child_count - 2));
  ...
  protected Declaration action_using_declaration(TreeNode root)
  {
    ...
    IDExpression expr = this.<IDExpression>doAction(root.getChild(child_count - 2));
  ...
  @SuppressWarnings("unchecked")
  public <T> T doAction(TreeNode root)
  {
    int action_code = -1;
    ...
    {
      case 4:
        return (T)action_ambiguity(root);
      ...
      case 107: // qualified_id
        return this.<T>doAction(root.getChild(0));

      case 1:
        return (T)action_abstract_declarator(root);
      case 2:
        return (T)action_access_specifier(root);
      case 11:
        return (T)action_assignment_expression(root);
      case 20:
        return (T)action_block_declaration(root);
      case 22:
        return (T)action_cast_expression(root);
      case 23:
        return (T)action_class_name(root);
      case 24:
        return (T)action_compound_statement(root);
      case 25:
        return (T)action_condition(root);
      case 26:
        return (T)action_conditional_expression(root);
      case 28:
        return (T)action_constant_initializer(root);
      case 30:
        return (T)action_conversion_function_id(root);
      case 32: // ctor_initializer
        return this.<T>doAction(root.getChild(1));
      case 33: // cv_qualifier
        return (T)action_specifier(root);
      case 35:
        return (T)action_decl_specifier(root);
      case 36:
      ...
      case 40:
        return (T)action_declarator_head(root);
      case 41:
        return (T)action_declarator_id(root);
      case 42:
        return (T)action_declarator_tail(root);
      case 45:
        return (T)action_direct_abstract_declarator(root);
      case 46:
        return (T)action_direct_declarator(root);
      case 48:
        return (T)action_enumerator(root); // enumerator_definition
      case 55:
        return (T)action_explicit_specialization(root);
      case 56:
        return (T)action_expression(root);
      case 58:
        return (T)action_expression_statement(root);
      case 59:
        return (T)action_for_init_statement(root);
      case 60:
        return (T)action_function_definition(root);
      case 61:
        return (T)action_function_prefix(root);
      case 62:
        return (T)action_specifier(root);
      case 66:
        return (T)action_id_expression(root);
      case 68:
        return (T)action_init_declarator(root);
      case 70:
        return (T)action_initializer(root);
      case 71:
        return (T)action_initializer_clause(root);
      case 73:
      ...
      case 76:
        return (T)action_linkage_specification(root);
      case 77:
        return (T)action_literal(root);
      case 80:
        return (T)action_mem_initializer(root);
      case 81:
        return (T)action_mem_initializer_id(root);
      case 83:
        return (T)action_member_declaration(root);
      case 85:
        return (T)action_member_declarator(root);
      case 88:
        return (T)action_member_specification(root);
      case 91:
        return (T)action_namespace_definition(root);
      case 98:
        return (T)action_operator_(root);
      case 99:
        return (T)action_operator_function_id(root);
      case 100:
        return (T)action_parameter_declaration(root);
      case 101:
        return (T)action_parameter_declaration_clause(root);
      case 103:
        return (T)action_pm_expression(root);
      case 104:
        return (T)action_postfix_expression(root);
      case 105:
        return (T)action_primary_expression(root);
      case 106:
        return (T)action_ptr_operator(root);
      case 110:
        return (T)action_scoped_class_name(root);
      case 112:
        return (T)action_scoped_unqualified_id(root);
      case 113:
        return (T)action_selection_statement(root);
      case 115:
        return (T)action_simple_declaration(root);
      case 116:
        return (T)action_simple_type_specifier(root);
      case 117:
        return (T)action_statement(root);
      case 119:
        return (T)action_specifier(root);
      case 120:
        return (T)action_template_argument(root);
      case 122:
        return (T)action_template_declaration(root);
      case 123:
        return (T)action_template_parameter(root);
      case 126:
        return (T)action_translation_unit(root);
      case 128:
        return (T)action_type_id(root);
      case 130:
        return (T)action_type_parameter(root);
      case 131:
        return (T)action_type_specifier(root);
      case 133:
        return (T)action_unary_expression(root);
      case 135:
        return (T)action_unqualified_id(root);
      case 136:
        return (T)action_using_declaration(root);
  ...
  protected Declarator mergeDeclaratorHeadAndTail(Declarator head, List<?> tail)
  {
    ...
    if (tail.get(0) instanceof ProcedureDeclarator)
    {
      ...
      List<Declaration> params = new LinkedList<Declaration>();
      for (Declaration d : pdecl.getParameters())
        params.add((Declaration)d.clone());
