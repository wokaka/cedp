...
public class CFGraph extends DFAGraph
{
	...
	protected Stack<List<DFANode>> break_link;    // List( from_list, to_node )
	protected Stack<List<DFANode>> continue_link; // List( from_list, to_node )
	protected Stack<List<DFANode>> switch_link;   // List( from_node, to_list )
	protected Map<Label,List<Object>> goto_link; // List( from_list, to_node )
	...
	public CFGraph(Traversable t, Class<? extends Traversable> supernode)
	{
		...
		goto_link = new HashMap<Label,List<Object>>();
	...
	public void normalize()
	{
		...
		do
		{
			...
			List<DFANode> data = node.getData("stmt");
			...
			{
				List<DFANode> symbol_exit = temp.getLast().getData("symbol-exit");

				if ( symbol_exit != null )
					symbol_exit.addAll(data);

				else
				{
					symbol_exit = new ArrayList<DFANode>();
					symbol_exit.addAll(data);
	...
	protected DFAGraph buildProcedure(Procedure proc)
	{
		...
		for ( Label l : goto_link.keySet() )
		{
			...
			List<DFANode> froms = (List<DFANode>)goto_link.get(l).get(0);
	...
	protected DFANode buildGoto(GotoStatement stmt)
	{
		...
		{
			goto_link.put(label, new ArrayList<Object>(2));
			goto_link.get(label).add(new ArrayList<Object>());
		}
		((List<DFANode>)goto_link.get(label).get(0)).add(ret);
	...
	protected DFANode buildLabel(Label label)
	{
		...
		{
			goto_link.put(label, new ArrayList<Object>(2));
			goto_link.get(label).add(new ArrayList<DFANode>());
		}
	...
	protected DFAGraph buildCompound(CompoundStatement stmt)
	{
		...
		while ( iter.hasNext() )
		{
			DFAGraph curr = buildGraph(iter.next());
		...
		if ( !stmt.getTable().isEmpty() )
		{
			...
			List<Object> symbol_exits = new ArrayList<Object>();
...
*/
	...
	protected static boolean isUnsafeExpansion(Traversable t)
	{
		...
		for ( int i=1; i < chunks.size(); ++i )
			if ( Tools.containsSideEffect(chunks.get(i)) )
				return true;
	...
	{
		List<Traversable> ret = new ArrayList<Traversable>();
	...
	protected static DFAGraph expandExpression(DFANode node)
	{
		...
		Class<? extends Traversable> before = t.getChildren().get(0).getClass();

		expandExpression(stmts, t);

		// Remove redundant repetition of lvalue.
		Class<? extends Traversable> after = t.getChildren().get(0).getClass();
	...
	protected static void expandAssignment
	...
	protected static void expandConditional
	...
	{
		Set<Symbol> lives = new HashSet<Symbol>(), kills = new HashSet<Symbol>();
		List<Traversable> stmt_list = stmts.getChildren();
		...
		for ( int i=stmt_list.size()-1; i>=0; --i )
		{
			...
			for ( Symbol var : Tools.getSymbols(stmts) )
		...
		while ( iter.hasNext() )
		{
			...
			for ( Symbol kill : kills )
				if ( Tools.containsSymbol(o, kill) )
				{
					iter.remove();
					break;
				}
