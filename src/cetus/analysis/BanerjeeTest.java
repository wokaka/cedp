...
public class BanerjeeTest implements DDTest
{
	...
	public BanerjeeTest (SubscriptPair pair)
	{
		...
		this.const1 = NormalExpression.getConstantCoefficient(subscript1, idlist);
		this.const2 = NormalExpression.getConstantCoefficient(subscript2, idlist);
		...
		for (Loop loop: loopnest)
		{
			...
			Expression c1 = NormalExpression.getCoefficient(subscript1, id);
			Expression c2 = NormalExpression.getCoefficient(subscript2, id);
	...
	public boolean testDependence(DependenceVector dependence_vector)
	{
		...
		long banerjeeUB=0;
		long diff = 0;
		Expression expr_diff = NormalExpression.subtract(const2, const1);
	...
	private void printDirectionVector(HashMap<Loop,Integer> dv, LinkedList<Loop> nest)
	{
		...
		for (int i=0; i< nest.size(); i++)
		{
			...
			Tools.print(DependenceVector.depstr[dv.get(loop)], 2);
		}
		Tools.println(")", 2);
	}
