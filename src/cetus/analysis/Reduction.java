...
*/
	...
	{ 
		 ...
		 */
		Map<Symbol, Set<Integer>> UseMap = Tools.getUseSymbolMap(istmt);
		Map<Symbol, Set<Integer>> DefMap = Tools.getDefSymbolMap(istmt);
		...
		Tools.mergeSymbolMaps(RefMap, DefMap);
		...
		while ( iter.hasNext() )
		{
			...
			if ( obj instanceof Expression )
			{
				...
				Tools.print("[expr] " + ++expr_cnt + " : " , 9);
				Tools.println(expr.toString() + " (" + expr.getClass().getName() + ")", 9);
				...
				{
					Set<Symbol> func_side_effect = Tools.getSideEffectSymbols((FunctionCall)expr);
		...
		for ( String op : rmap.keySet() )		// Foreach reduction operator ("+" and "*")
		{
			for (Expression candidate : rmap.get(op))
			{
				...
				Tools.println("candidate: " + candidate.toString(), 2);
				Symbol candidate_symbol = Tools.getSymbolOf(candidate);
				if ( RefMap.get(candidate_symbol) == null ) continue;
				if (!RefMap.get(candidate_symbol).isEmpty())
				{
					Tools.println("  " + candidate + " is referenced in the non-reduction statement!", 2);
					remove_flag = true;
				}
				if ( alias != null && alias.isAliased(null, candidate_symbol, RefMap.keySet()) )
				{
					Tools.println("  " + candidate + " is Aliased!", 2);
					remove_flag = true;
				}
				if ( side_effect_set.contains(candidate_symbol) )
				{
					Tools.println("  " + candidate + " has side-effect!", 2);
					remove_flag = true;
				}
				if (candidate instanceof ArrayAccess && istmt instanceof ForLoop)
				{
					...
					{
						Tools.println("No self-carried output dependence in " + candidate.toString(), 2);
						Tools.println("loop: " + loop.toString(), 2);
						reduction_set.remove(candidate);	
					}
					 */
	
					if ( simple_self_dependency_check((ArrayAccess)candidate, (ForLoop)istmt) ) {
						Tools.println("No self-carried-output dependence in " + candidate, 2);
						remove_flag = true;
					}
				}
...
/*				if (candidate instanceof ArrayAccess && istmt instanceof ForLoop)
	...
	private void findReduction(UnaryExpression expr, Map<String, Set<Expression>> rmap, Map<Symbol, Set<Integer>> cmap)
	{
		...
		if (isReduction)
		{
			...
			add_to_cmap(cmap, Tools.getSymbolOf(lhse), System.identityHashCode(lhse));
			Tools.println("candidate = ("+reduction_op+":"+lhse.toString()+")", 2);
		}
	}	
		
	private void findReduction(AssignmentExpression expr, Map<String, Set<Expression>> rmap, Map<Symbol, Set<Integer>> cmap)
	{
		boolean isReduction = false;
		...
		{
			add_to_rmap(rmap, reduction_op, lhse);
			for ( Integer hash_code : Tools.findUniqueExpressionList(expr, lhse) )
			{
				add_to_cmap(cmap, Tools.getSymbolOf(lhse), hash_code);
			}
			Tools.println("candidate = ("+reduction_op+":"+lhse.toString()+")", 2);
		}
	}
