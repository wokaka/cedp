...
//		debug = Integer.valueOf(Driver.getOptionValue("verbosity")).intValue();
	...
	public void analyzeProcedure(Procedure proc)
	{
		...
		Tools.printlnStatus(tag+" Procedure \""+proc.getName()+"\" ...", 1);
	...
	private void addAnnotation(Procedure proc)
	{
		...
		for ( Loop loop : pri_map.keySet() )
		{
			...
			for ( Symbol var : keys )
			{
				Set<Symbol> alias_set =
					alias_analysis.get_alias_set((Statement)loop, var);

				if ( alias_set == null )
					continue;

				for ( Symbol aliased : alias_set )
				{
					if ( aliased == null )
					{
						alias_result = 1;
					...
					else if ( var != aliased )
					{
						...
						Tools.printlnStatus(tag+"  Removing aliased variable ("+
							var+","+aliased+")", 1);
		...
		for ( Loop loop : pri_map.keySet() )
		{
			...
			{
				if ( Tools.isGlobal(var, loop_stmt) )
				{
					loop_pri_set.remove(var);
	...
	private CFGraph buildLoopGraph()
	{
		...
		Tools.printlnStatus(ret.toDot("range,ir,tag", 2), 5);
	...
	private void addUseFromDeclaration()
	{
		...
		RangeDomain rd = range_map.get(current_loop);
		if ( rd == null )
			rd = new RangeDomain();
		else
...
*/
	...
	private void collectPrivateSet(CFGraph g)
	{
		...
		while ( iter.hasNext() )
		{
			...
			if ( o instanceof Loop )
			{
				Set<Symbol> inner_pri = new LinkedHashSet<Symbol>(pri_map.get(o));
		...
		Set<Symbol> local_set =
			Tools.getVariableSymbols((SymbolTable)current_loop.getBody());
		...
		Tools.printlnStatus(tag+" DEF = "+def_map.get(current_loop), 2);
		Tools.printlnStatus(tag+" USE = "+use_map.get(current_loop), 2);
		Tools.printlnStatus(tag+" PRI = "+pri_map.get(current_loop), 2);
	}
	...
	private void removeUserTypes(Set<Symbol> vars)
	{
		...
		for ( Symbol var : keys )
		{
			...
			List<Specifier> types = var.getTypeSpecifiers();
			if ( types != null )
			{
	...
	private void removeMayUsedVariables(Set<Symbol> vars)
	{
		...
		while ( iter.hasNext() )
		{
			...
			for ( Symbol param : params )
			{
				...
				{
					List<Specifier> spec = param.getTypeSpecifiers();
		...
		if ( fc != null )
		{
			...
			for ( Symbol var : keys )
			{
				if ( Tools.isGlobal(var, current_loop) )
					vars.remove(var);
			}
		}
	}
	...
	private void aggregateDef()
	{
		...
		RangeDomain entry_range = range_map.get(current_loop);
		if ( entry_range == null )
		...
		Tools.printlnStatus(tag+" DEFS = "+defs+" under "+exit_range, 2);
	}
	...
	private void aggregateUse()
	{
		...
		Tools.printlnStatus(tag+" USES = "+my_use+" under "+exit_range, 2);
	}
	...
	private void getRangeDomain(CFGraph g)
	{
		...
		for ( Integer order : work_list.keySet() )
		{
			...
			{
				RangeDomain range = range_map.get(current_loop);
