			...
			// check if array loop carried dependences exist
				...
				for (DDGraph.Arc dep : loop_carried_deps)
				{
					if (dep.isCarried(loop))
					{
						...
						Symbol dep_symbol = Tools.getSymbolOf(dep_access);
	 ...
	 */
        private void parallelizeLoopNest(Loop enclosing_loop)
        {
                ...
                List<Loop> eligible_loops = 
                        LoopTools.extractOutermostDependenceTestEligibleLoops(enclosing_loop);
                        ...
                        for (Loop l : contained_nest)
                        {
                                ...
                                else
                                {
                                        ...
                                        while (iter.hasNext())
                                        {
                                                ...
                                                if ((row.getDirectionVector().vector).containsKey(l) &&
                                                                (row.getDirectionVector().getDirection(l)
                                                                                != DependenceVector.equal) &&
                                                                                (row.getDirectionVector().getDirection(l)
                                                                                                != DependenceVector.nil))
                                                {
                                                        ArrayAccess src_access = row.getSource().getArrayAccess();
                                                        Symbol src_symbol = Tools.getSymbolOf(src_access);
							ArrayAccess sink_access = row.getSink().getArrayAccess();
							Symbol sink_symbol = Tools.getSymbolOf(sink_access);
							...
							if (serialize)
							{
                                                                 ...
                                                                 */
                                                                if (row.getDirectionVector().getDirection(l)
	...
	private void reportParallelization()
	{
		...
		while ( iter.hasNext() )
		{
			...
			if ( o instanceof ForLoop )
			{
				...
				if ( Tools.containsClass(for_loop, FunctionCall.class) )
					out.append(", FunctionCall");
				if ( Tools.containsIO(for_loop) )
					out.append(", I/O");
