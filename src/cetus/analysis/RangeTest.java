...
/-----------------------------------------------------------------------------*/
	...
	private void setLoops(SubscriptPair pair)
	{
		...
		for ( Loop loop : common_loops )
		{
			...
			if ( Tools.containsSymbol(f, index_sym) ||
					Tools.containsSymbol(g, index_sym) )
	...
	private int getMonoState(Expression e, Expression ref, Loop loop)
	{
		...
		Expression next = Tools.replaceSymbol(e, index.getSymbol(),
	...
	{
		if ( verbosity >= 2 )
			Tools.printlnStatus(tag+"range("+e+") w.r.t. "+loopsToString(loops), 2);
		...
		{
			if ( verbosity >= 2 )
				Tools.printlnStatus(tag+"  = "+ret+" from the cache", 2);
			return ret;
		}
...
*/
	...
	public boolean testDependence(DependenceVector dvec)
	{
		...
		if ( verbosity >= 1 )
			Tools.printlnStatus(tag+"under "+dvec+" --> "+ret, 1);
		return ret;
	}
	...
	private void solve()
	{
		...
		for ( int i=common_loops.size()-1; i>=0; i-- )
		{
			...
			if ( verbosity >= 2 )
				Tools.printlnStatus(tag+"for "+loopToString(loop), 2);
			...
			Iterator<Loop> perm_iter = permuted.iterator();
			while ( perm_iter.hasNext() && !placed )
			{
				Loop perm_loop = perm_iter.next();
		...
		if ( verbosity >= 1 )
			Tools.printlnStatus(tag+this, 1);
	}
	...
	private void setIndependentVectors()
	{
		for ( Loop loop : parallel_loops.keySet() )
		{
			...
			for ( int i=0; i<loop_id; i++ )
			{
				...
				Set<Symbol> symbols_in_pair = Tools.getAccessedSymbols(f);
				symbols_in_pair.addAll(Tools.getAccessedSymbols(g));
	...
	private boolean test1(Loop loop, Set<Loop> loops)
	{
		...
		if ( verbosity >= 2 )
			Tools.printlnStatus(tag+"test1 = "+ret, 2);
		return ret;
	}
	...
	)
	{
		...
		{	
			if ( verbosity >= 2 )
				Tools.printlnStatus(tag+"max1 = "+max1+", min2 = "+min2, 2);
			return false;
		}
...
//		Symbol index_sym = index.getSymbol();
		...
		*/
		...
		if ( verbosity >= 2 )
			Tools.printlnStatus(tag+"compare "+max1+" "+rel+" "+min2, 2);
		return rel.isLT();
	}
	...
	private boolean test2(Loop loop, Set<Loop> loops)
	{
		...
		if ( verbosity >= 2 )
			Tools.printlnStatus(tag+"test2 = "+ret, 2);
		return ret;
	}
	...
	)
	{
		...
		RangeDomain rd = (RangeDomain)common_range.clone();
		...
		min2 = Tools.replaceSymbol(min2, id, new_index);
